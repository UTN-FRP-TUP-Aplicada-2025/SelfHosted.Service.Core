...
**Lo que este intake no decide, actualizado en la versión 3.2.** El documento de entrada abre veintiocho decisiones. Hasta la versión 3.1 **sólo cuatro estaban cerradas** —las cuatro de arriba—, **ocho tenían propuesta escrita y sin confirmar** como especificaciones de integración `[D-i]` (`DI-17` a `DI-24`) y **diecisiete seguían abiertas sin propuesta cerrada**.

**El 2026-07-30 el agente humano del proyecto cerró seis más**, en la ronda de decisiones que desbloquea la reconstrucción de la maqueta de la Fase B2: confirmó `DI-17`, `DI-18` y `DI-19` —que corresponden a `Q-1`, `Q-2` y `Q-3`— y resolvió `Q-15`, `Q-17` y `Q-27`. **`Q-6` cae con ellas por arrastre**, porque su propia fila declaraba que `Q-15` la responde. La aritmética queda así, y suma las veintinueve filas sobre los veintiocho identificadores: **once cerradas**, **cinco con propuesta escrita y sin confirmar** (`DI-20` a `DI-24`) y **trece abiertas sin propuesta cerrada**, enumeradas en la tabla de §19 con su consumidor identificado y **sin valor supuesto**. Ninguna de las trece se presume resuelta en este documento.
...
**Estado de esta nota, cerrado en la versión 3.2.** El reparto de siete vías sobre cinco variantes, la separación de imagen pública y privada, y la existencia del servicio sin origen como nodo borrador entraron en la versión 2.4 como **propuesta del integrador sin confirmar**: son `DI-17`, `DI-18` y `DI-19` de §19. **El agente humano del proyecto las confirmó las tres el 2026-07-30**, en la ronda de decisiones que desbloquea la reconstrucción de la maqueta. Dejan de ser `[D-i]` revisables y pasan a `[D]`: la cadena las consume como decisión cerrada y ningún artefacto aguas abajo necesita declararlas provisionales.
...
| Campo | Qué es | Estado |
|---|---|---|
| `origen.tipo` | De tres valores planos a **cinco variantes discriminadas**, cada una con sus campos propios | **[D]** `DI-17`, `DI-18`, confirmadas el 2026-07-30 |
...
| `estado` | `borrador`, `pendiente-de-aplicar` o `aplicado`. **Ortogonal** al estado del despliegue, que sigue siendo el de [E-17](#2017--e-17--ciclo-de-vida-del-despliegue-y-correspondencia-con-el-motor) | **[D]** `DI-19`, confirmada el 2026-07-30 |
...
| `procedencia` | Huella de auditoría de la **vía de alta**: adopción o catálogo, con qué se instanció y cuándo. **No es configuración y no vive en `origen`** | **[D]** `DI-17`, confirmada el 2026-07-30; su contenido para el catálogo es **[D]** por D-14 |
| `verificaciones` | Estado, momento e informe de las **dos verificaciones**, la del origen y la de la configuración, que son operaciones distintas con informes distintos | **[D]** `DI-17`, confirmada el 2026-07-30 |
...
| RN-40 **[D]**, `DI-17` confirmada el 2026-07-30 · **nueva el 2026-07-29** | **Una imagen marcada como conservada no se limpia, y una imagen ajena no se toca.** Ninguna operación de limpieza puede eliminar una imagen con la marca de conservada, ni una que no lleve la marca de pertenencia del producto, aunque ningún despliegue activo la referencie. Es lo que hace practicable cualquier limpieza en un almacén de imágenes que el producto **comparte** con el parque preexistente y con el ejecutor de integración continua | Toda operación de limpieza de imágenes | La imagen se excluye de la operación y el informe declara por qué. **Actualizado el 2026-07-30:** `Q-15` y `Q-17` quedaron decididas —el despliegue registra el digesto y la limpieza es **sugerida**—, de modo que la regla ya tiene con qué resolver el uso de cada imagen y cuál es el momento en que la protección se ejerce. Siguen abiertas `Q-16`, `Q-18`, `Q-19`, `Q-20` y `Q-21`: la regla declara la protección y no quién puede poner la marca ni con qué alcance |
...
**Qué quedó decidido y qué queda abierto, actualizado en la versión 3.2.** El modelo de arriba era la **forma mínima** que las decisiones abiertas necesitaban para poder tomarse. Siete pendientes de decisión de §19 viven en este anexo; **el agente humano del proyecto cerró dos el 2026-07-30** y las cinco restantes siguen abiertas, sin valor supuesto.

| Pendiente | Estado | Qué se decidió, o qué falta decidir |
|---|---|---|
| `Q-15` | **Decidida el 2026-07-30: sí** | El despliegue **registra el digesto** de la imagen que usó. Es la pieza que habilita el resto de este anexo: el bloque `imagen` del despliegue tiene quien lo escriba, el indicador de uso de cada imagen es resoluble, y «volver al despliegue N» pasa a ser una operación posible. Cierra además `Q-6`, que preguntaba por el commit: el digesto lo cubre |
| `Q-16` | Abierta | Si las imágenes construidas llevan **marca de pertenencia**. Sin ella, `gestionadaPorElPanel` no se puede determinar y ninguna limpieza es segura |
| `Q-17` | **Decidida el 2026-07-30: sugerida** | La limpieza **no corre sola y no se dispara sólo a pedido**: el sistema detecta espacio recuperable y lo propone; el usuario confirma. Queda por especificar en `03-UX-UI-DX` dónde aparece la sugerencia y con qué umbral, que es trabajo de esa categoría y no una pendiente de este intake |
| `Q-18` | Abierta | Si la limpieza tiene **ámbito propio** de credencial, distinto del de desplegar. Por el principio de ámbito mínimo: un automatismo que despliega no debe poder borrar |
| `Q-19` | Abierta | Si existe la operación de **volver a un despliegue anterior**. `Q-15` la volvió técnicamente posible; lo que falta es decidir si se ofrece |
| `Q-20` | Abierta | Al volver, si la política de actualización **pasa a fijada sola o se pregunta**. Depende de `Q-19` |
| `Q-21` | Abierta | Si la imagen conservada queda **protegida de la limpieza**, quién puede marcarla y con qué alcance. Sin ella, la limpieza borra exactamente la versión a la que se quería poder volver |

**Lo que la decisión de `Q-15` habilitó, y lo que no.** La versión 2.4 declaraba que «las siete se cierran con `Q-15`», porque registrar el digesto por despliegue es la pieza única de la que dependen saber qué corre, decidir qué se descarta y volver a una versión anterior. La formulación era imprecisa y conviene corregirla acá: `Q-15` es **condición** de las otras seis, no su respuesta. Decidida en positivo, las cinco que siguen abiertas pasan de imposibles de responder a respondibles, y ninguna queda respondida por arrastre. Ninguna se presume en este documento.
...
| Pendientes de decisión | Decisiones de producto que **nadie tomó** y que este intake deliberadamente no toma. Cinco de la Fase A, más **diecisiete** incorporadas en la versión 2.4 por la redefinición del alta de servicio: **veintidós** en total, de las cuales **cinco están cerradas** desde la versión 3.2 —una lo estaba ya, y `Q-15`, `Q-17`, `Q-27` y `Q-6` cerraron el 2026-07-30— y **diecisiete siguen abiertas** | La tabla que sigue, en sus dos bloques |
...
**Diecisiete pendientes nuevas, incorporadas en la versión 2.4; trece siguen abiertas.** Son las que la redefinición del alta de servicio abrió. Este intake **no toma ninguna que siga abierta**, y ninguna lleva valor supuesto: donde el dato falta, la fila lo declara. Se agrupan por bloque para que la conversación con el agente humano del proyecto sea abordable, que es el mismo criterio con el que §24.1 agrupó las catorce especificaciones sin revisar.

**Cuatro de las diecisiete quedaron cerradas el 2026-07-30.** Tres las decidió el agente humano del proyecto en la ronda que desbloquea la reconstrucción de la maqueta —`Q-15`, `Q-17` y `Q-27`— y la cuarta, `Q-6`, cae por arrastre porque su propia fila declaraba que `Q-15` la responde. Sus filas **se conservan en la tabla con su resolución** en lugar de retirarse, por dos motivos: los artefactos aguas abajo citan estos identificadores y un ancla que desaparece rompe la traza, y el estado de una pendiente cerrada es información que la cadena consume.
...
| `Q-6` | Si se registra **qué commit construyó cada despliegue** | E-3, E-23 | **Cerrada el 2026-07-30 por arrastre de `Q-15`**, tal como esta fila ya lo anticipaba: el digesto cubre qué versión corre. No requiere decisión propia |
...
| `Q-15` | Si el despliegue registra el **digesto** de la imagen | E-23, E-3 | **Decidida el 2026-07-30: sí.** El despliegue registra el digesto. Era la pieza de mayor palanca de las diecisiete: habilita el anexo E-23, cierra `Q-6` y vuelve respondibles `Q-19` a `Q-21`. Consume la decisión `02-Especificacion-Funcional` en el modelo conceptual de la imagen, y `03-UX-UI-DX` en el indicador de uso de `SUP-18` |
...
| `Q-17` | Si la limpieza de imágenes es **manual, sugerida o programada** | E-23 | **Decidida el 2026-07-30: sugerida.** El sistema detecta espacio recuperable y lo propone; el usuario confirma. No corre sola ni espera únicamente a que se la pida. Llenar el disco es el modo de falla más probable del servidor de referencia, y la forma sugerida lo previene sin borrar por su cuenta. `03-UX-UI-DX` especifica dónde aparece la sugerencia y con qué umbral |
...
| `Q-27` | Si hay alguna forma de **explorar un registro de imágenes**, o conocer la dirección de la imagen es requisito del usuario | §4, E-2 | **Decidida el 2026-07-30: hay exploración.** El panel ofrece explorar el registro configurado, y conocer la dirección de la imagen deja de ser requisito del usuario. Era el primer minuto de uso: sin catálogo de fábrica y sin exploración, quien no sabía la dirección no tenía camino. **Consecuencia declarada: es una superficie nueva**, que `03-UX-UI-DX` especifica con wireframe propio antes de que la maqueta la construya, y que suma alcance al Alcance 1 |
...
**Cuatro decisiones condicionaban la forma de una pantalla**, y por lo tanto había que cerrarlas antes de rehacer la maqueta; el resto condiciona datos o reglas y puede resolverse después. No eran cuatro de esta tabla: **tres eran especificaciones de integración `DI-17` a `DI-19`** —aplicadas y esperando confirmación— y **una era `Q-27`**, abierta. **Las cuatro quedaron cerradas el 2026-07-30**, junto con `Q-15` y `Q-17`, que bloqueaban la construcción de `SUP-18`:

| Decisión | Estado | Qué pantalla condiciona |
|---|---|---|
| `DI-17` · siete vías sobre cinco variantes | **Confirmada el 2026-07-30.** Pasa a `[D]` | El alta entera, `SUP-17`. La superficie se maqueta con las siete vías |
| `DI-18` · separar imagen pública de privada | **Confirmada el 2026-07-30.** Pasa a `[D]` | El paso de credencial de registro existe, y el registro es dirección y no selector |
| `DI-19` · el servicio sin origen | **Confirmada el 2026-07-30.** Pasa a `[D]` | El lienzo admite el nodo en estado `borrador`, no desplegable y fuera del conjunto de cambios |
| `Q-27` · exploración de registro de imágenes | **Decidida el 2026-07-30: hay exploración** | Superficie nueva, con wireframe propio en `03-UX-UI-DX` |
| `Q-15` · digesto por despliegue | **Decidida el 2026-07-30: sí** | Desbloquea el indicador de uso de `SUP-18` |
| `Q-17` · disparo de la limpieza | **Decidida el 2026-07-30: sugerida** | Desbloquea el encabezado de `SUP-18` |

**Con las seis cerradas, la reconstrucción de la maqueta deja de estar bloqueada.** `PA-15` se resolvió en la misma ronda a favor de conservar `SUP-17` como superficie propia, de modo que no queda ninguna reubicación de contenido pendiente en `03-UX-UI-DX`.

**Especificaciones de integración (marcador `[D-i]`), veinticuatro, de las que diecinueve siguen pendientes de confirmación.** Son puntos que el orquestador resolvió al integrar las decisiones del agente humano, porque sin ellos las decisiones no eran implementables. Las dieciséis primeras vienen de la versión 1.2 y de sus pasadas; `DI-17` a `DI-24` se incorporaron en la versión 2.4 con la redefinición del alta de servicio. **Están aplicados en este documento y la cadena puede consumirlos**; los que siguen marcados `[D-i]` no los decidió el agente humano del proyecto y deben declararse revisables aguas abajo. **`DI-17`, `DI-18` y `DI-19` dejaron de serlo el 2026-07-30**: el agente humano las confirmó y pasan a `[D]`, de modo que la cadena las consume como decisión cerrada. La convención `[D-i]` está definida en la sección «Procedencia de este intake y convención de marcadores», y el reparto de autoría frente a cada decisión está en «Qué decidió el agente humano y qué derivó el integrador».
...
| DI-17 · **confirmada el 2026-07-30, pasa a `[D]`** | **El reparto de siete vías de alta sobre cinco variantes discriminadas de origen**, con la vía como eje que no se persiste y la procedencia como su huella de auditoría; el estado del servicio con sus tres valores ortogonal al del despliegue; las **dos verificaciones** separadas con sus cuatro criterios; la reformulación de RN-08 por variante; y las reglas RN-38 y RN-40 | Redefinición del alta de servicio, 2026-07-29 | §4, E-2, E-7, E-16 (RN-08, RN-38, RN-40), E-23 |
| DI-18 · **confirmada el 2026-07-30, pasa a `[D]`** | Que **la imagen pública y la privada se separen en la interfaz sin duplicar el modelo**: el registro pasa de selector a dirección y aparece la credencial, y `credencialRegistroId` y `credencialRepositorioId` son entidades distintas | Redefinición del alta de servicio, 2026-07-29 | §4, E-2 |
| DI-19 · **confirmada el 2026-07-30, pasa a `[D]`** | Que el **servicio sin origen** exista como nodo en estado `borrador`, no desplegable y fuera del conjunto de cambios pendientes, y que sea el alta detenida en el paso del nombre en lugar de una vía con mecánica propia | Redefinición del alta de servicio, 2026-07-29 | §4, E-2 |
...
**Estado de revisión, incorporado en la versión 2.0 y actualizado en la 2.4 y en la 3.2.** Dos de las veinticuatro fueron revisadas y aprobadas por el agente humano del proyecto al cierre de la Fase A —DI-01 y DI-03—, y **tres más el 2026-07-30**: DI-17, DI-18 y DI-19, confirmadas en la ronda que desbloquea la reconstrucción de la maqueta. Las catorce restantes de la tanda original siguen sin revisar y **cinco de la versión 2.4 siguen sin revisar** —DI-20 a DI-24—, de modo que **diecinueve de veinticuatro** esperan confirmación. El detalle, con las catorce agrupadas por lo que deciden para que la revisión sea abordable, está en [§24.1](#241-las-dieciséis-especificaciones-derivadas-y-su-estado-de-revisión).
...
|---|---|---|---|
| 3.3 | 2026-07-30 | **Corrección del hallazgo `P2-1` del informe `B2-Retroalimentacion-Decisiones-2026-07-30-r1.md`**, audit independiente del paso 6 de la Fase B2. Sube **minor**: corrige un conteo y no toca ninguna decisión de producto ni ninguna sección de contenido. La fila 3.2 declaraba que los marcadores `[D-i]` de `DI-17` a `DI-19` pasaban a `[D]` en «seis ocurrencias» y enumeraba §20.2.1 con «tres filas»; **en disco son ocho ocurrencias y §20.2.1 tiene cuatro filas** —`origen.tipo`, `estado`, `procedencia` y `verificaciones`—. El error estaba en la enumeración de la propia fila, no en el documento: las ocho ocurrencias estaban correctamente convertidas. Se corrige el conteo y se separa lo que la fila mezclaba: las **ocho ocurrencias de marcador** por un lado, y la **nota de estado de §4**, que no lleva marcador, por otro. **Se corrige emitiendo versión y no editando la fila 3.2 en su lugar**, porque este documento está en estado `Aprobado` y la política de versionado de `Master-Prompt.md` §5 manda subir versión y archivar desde que un documento deja de ser borrador. Es exactamente el defecto que la fila 3.1 registra haber cometido y deshecho, y no se repite. La versión 3.2 queda archivada en `SDD/Intake/_legacy/2026-07-30/PRODUCT-INTAKE-SelfHosted-Service-v3.2.md`. | Orquestador SDD |
| 3.2 | 2026-07-30 | **Consolidación de la ronda de decisiones del agente humano del proyecto del 2026-07-30**, por el caso (a) de escritura de `Master-Prompt.md` §13 regla 2: respuestas del humano a las preguntas que bloqueaban el paso 5 de la Fase B2. Sube **minor** por la regla 4: incorpora el estado de decisiones que el documento ya enumeraba como abiertas y **no reescribe ninguna sección aprobada**; ninguna regla de negocio, ningún anexo de datos, ningún umbral y ninguna capacidad cambian de contenido. **Se emite como entrada única y no como una por sección**, con la regla 5 leída sobre el evento y no sobre el archivo: las seis decisiones son una sola respuesta del humano y partirla en seis filas haría ilegible la traza de qué se contestó cuándo; las secciones alcanzadas se enumeran acá una por una. **Seis decisiones cerradas.** `DI-17`, `DI-18` y `DI-19` **confirmadas**: el reparto de siete vías de alta sobre cinco variantes discriminadas, la separación de imagen pública y privada en la interfaz, y el servicio sin origen como nodo `borrador`. Dejan de ser `[D-i]` revisables y pasan a `[D]` en sus seis ocurrencias de marcador —§4 nota de los dos ejes, §20.2.1 en tres filas, §20.2.5 y E-16 en RN-08, RN-38 y RN-40—. `Q-15` **decidida en positivo**: el despliegue registra el digesto de la imagen. `Q-17` **decidida**: la limpieza es **sugerida**, ni manual ni programada. `Q-27` **decidida**: hay exploración de registro de imágenes, con la consecuencia declarada de que es una superficie nueva que `03-UX-UI-DX` debe especificar. **`Q-6` cierra por arrastre**, porque su fila ya declaraba que `Q-15` la responde. **Aritmética actualizada en las tres secciones que la declaran**: «Lo que este intake no decide» pasa a once cerradas, cinco con propuesta escrita sin confirmar y trece abiertas; §19 pasa de veintidós pendientes con una cerrada a veintidós con cinco cerradas; la tabla de `[D-i]` pasa de veintidós a diecinueve esperando confirmación. **Las filas de las cuatro pendientes cerradas se conservan en su tabla con su resolución** en lugar de retirarse, porque artefactos aguas abajo las citan por identificador y un ancla que desaparece rompe la traza. **Una imprecisión propia se corrige en lugar de arrastrarse**: E-23 declaraba que «las siete se cierran con `Q-15`», cuando `Q-15` es su condición y no su respuesta; `Q-16`, `Q-18`, `Q-19`, `Q-20` y `Q-21` siguen abiertas y ninguna queda respondida por arrastre. **`PA-15` resuelto** a favor de conservar el alta de servicio como superficie propia `SUP-17`, lo que no altera este documento y se registra acá porque cierra el conflicto que `03-UX-UI-DX` declaraba. Origina el cambio: la reanudación de la Fase B2 y la reconstrucción de la maqueta desde la especificación corregida. | Orquestador SDD |
