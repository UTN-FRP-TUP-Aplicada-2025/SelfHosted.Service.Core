> **Bloque de archivado.** Estado: `Superado`. Esta es la copia completa y autocontenida de la versión 2.0 de `SOLUTION-INTAKE-SelfHosted-Service-Core.md`, tomada el 2026-07-29 antes de aplicar las decisiones D-D, D-E (con E-1 y E-2), D-F y F-1 del agente humano del proyecto. La versión vigente es `../../SOLUTION-INTAKE-SelfHosted-Service.md`, que además cambió de nombre lógico: la decisión F-1 (b) derivó el slug documental del nombre de producto declarado, `SelfHosted Service`, y `Nombre-Solucion` pasó de `SelfHosted-Service-Core` a `SelfHosted-Service`. El cuerpo de este snapshot no se modifica: sus nombres, sus enlaces y su estado quedan como estaban.

# SOLUTION-INTAKE-SelfHosted-Service-Core
...
| Documento | `SOLUTION-INTAKE-SelfHosted-Service-Core.md` |
| Versión | 2.0 |
| Fecha | 2026-07-28 |
...
- [Parte B — Composición de la solución](#parte-b--composición-de-la-solución)
  - [§13 Proyectos de código de la solución](#13-proyectos-de-código-de-la-solución)
...
- [Parte C — Técnica por proyecto de código](#parte-c--técnica-por-proyecto-de-código)
  - [§17 Bloque técnico por proyecto de código](#17-bloque-técnico-por-proyecto-de-código)
    - [§17.1 `SelfHosted-Web`](#171-selfhosted-web)
    - [§17.2 `SelfHosted-Application`](#172-selfhosted-application)
    - [§17.3 `SelfHosted-Infrastructure`](#173-selfhosted-infrastructure)
    - [§17.4 `SelfHosted-Domain`](#174-selfhosted-domain)
...
**Advertencia para la Fase B.** El problema **escala con la cantidad de artefactos**, y la Fase B lo multiplica por doce categorías y cuatro proyectos de código: un enunciado del intake que hoy tiene cinco réplicas internas va a tener decenas repartidas en documentos que ya no están en este archivo. La «Trazabilidad downstream» de este intake sirve para eso **leída en el otro sentido**: no sólo para saber qué documento deriva de qué sección, sino **qué documentos hay que revisar cuando un enunciado de esa sección cambia de forma**. El orquestador de la Fase B debería tratarla como índice de propagación y no sólo como índice de origen.
...
|---|---|---|
| Matriz de navegadores de escritorio soportados, con familias y versiones mínimas | §17.1 P.9, fila de navegador | **Abierto.** Ningún valor propuesto. Condiciona `03-UX-UI-DX` de SelfHosted-Web y la fila correspondiente de `Compatibilidad-Plataformas`, donde queda declarada como brecha. Su desarrollo completo —por qué pesa más bajo Blazor Interactive Server, qué componentes del riesgo resuelve la red local y cuáles no, y la recomendación registrada— está en [§24.2](#242-la-matriz-de-navegadores-única-pendiente-que-espera-decisión) |
...
|---|---|---|
| SelfHosted-Domain | Entidades, objetos de valor, invariantes del modelo (I1 a I10, enumeradas en §17.4 P.2) y reglas de negocio (RN-01 a RN-37, transcriptas en el anexo [E-16](#2016--e-16--catálogo-de-reglas-de-negocio-rn-01-a-rn-37)) verificables sin infraestructura | Application, Infrastructure, Web |
...
| Nivel | Proporción | Proyecto de código | Framework | Qué cubre |
|---|---|---|---|---|
| Unitarias | 100 % | `SelfHosted.Domain.Tests` | xUnit | Invariantes del modelo (I1 a I10) y reglas de negocio (RN-01 a RN-37), **sin infraestructura** **[E]**, en particular la regla de conflicto de direcciones IP |
...
Los sesenta y cuatro casos del anexo [E-22](#2022--e-22--casos-de-prueba-derivados-de-las-configuraciones-reales) son el punto de partida de esta batería: cada uno lleva su entrada concreta y su resultado esperado, y varios usan como dato de entrada las configuraciones reales del anexo [E-20](#2020--e-20--configuraciones-reales-de-contenedor-ofuscadas), de modo que las pruebas se escriben contra formas que ya se sabe que existen en un servidor real. Las treinta y siete reglas del anexo [E-16](#2016--e-16--catálogo-de-reglas-de-negocio-rn-01-a-rn-37) se traducen cada una en al menos una prueba **[E]**: la fuente declara ese catálogo pensado exactamente para eso, con el momento de validación y la respuesta esperada de cada regla, que son el enunciado y la aserción de la prueba. Cada etapa que introduzca una regla nueva debe traer su prueba: es el criterio de cierre de etapa.
...
-- SelfHosted-Domain y de SelfHosted-Infrastructure, con el modelo de dominio
...
Citado desde §14, §17.1 P.3, §17.4 P.5, §17.4 P.6 y §17.4 P.10. Procedencia: `Analisis-Final-Integrado.md`, líneas 2183–2210. Estado: `declarado`. La fuente lo declara pensado para que **cada regla se traduzca en una prueba automatizada**: es el insumo directo de la categoría `08-Calidad-Y-Pruebas` y de la batería de `SelfHosted.Domain.Tests`.
...
Citado desde §6 (flujo 1), §15 (etapa `b`) y §16 (`SDD/Maquetas/SelfHosted-Web/`). Procedencia: `Analisis-Final-Integrado.md`, líneas 1688–1833. Estado: `declarado`. Es el insumo de la categoría `03-UX-UI-DX` de SelfHosted-Web y de la maqueta de validación visual contra la que se valida la etapa `b`.
...
|---|---|---|---|---|---|
| SelfHosted-Web | Linux Debian 13, kernel 6.12 | .NET 10 | Docker 26.x con `compose` v5 y `buildx`, por socket del host | Delegado en Infrastructure | Navegador de escritorio con soporte de WebSockets, sin familia declarada |
| SelfHosted-Application | Indiferente | .NET 10 | No aplica | No aplica | No aplica |
| SelfHosted-Infrastructure | Linux Debian 13, kernel 6.12 | .NET 10 | Docker 26.x; el cliente declara soporte de la interfaz del motor v29.4.1 | SQLite embebido en el proveedor de acceso a datos de .NET 10, con diario WAL | No aplica |
| SelfHosted-Domain | Indiferente | .NET 10 | No aplica | No aplica | No aplica |
...
| AT-11 | SemVer 2.0.0, único para los cuatro proyectos de código, porque se despliegan como un único artefacto | La versión calculada en el pipeline es única |
...
| DoD-03 | La cobertura alcanza el mínimo de cada proyecto de código: dominio 90 % de líneas y 85 % de ramas; aplicación 80 % y 70 %; interfaz 60 % y 50 %; adaptadores 55 % y 45 % | Sí |
...
- [x] §24 declara qué quedó abierto al cierre de la Fase A: las catorce especificaciones de integración sin revisar, la matriz de navegadores como única decisión de producto que espera respuesta, y los tres objetos declarados y no diseñados que la Fase C recibe como trabajo propio.
...
|---|---|---|
| Matriz de navegadores de escritorio soportados, con familias y versiones mínimas · **es la única que espera decisión antes de la Fase B**, ver [§24.2](#242-la-matriz-de-navegadores-única-pendiente-que-espera-decisión) | §17.1 P.9 | `03-UX-UI-DX` de SelfHosted-Web, y `Compatibilidad-Plataformas` de `00-Contexto`, donde la fila queda declarada como brecha. Alcanza además a `08-Calidad-Y-Pruebas` y a `09-Devops`, que necesitan la matriz para declarar contra qué se verifica |
| Asignación de F-23 (variables compartidas), F-24 (referencias entre variables) y F-25 (higiene del modelo) a un alcance y a un corte vertical concreto | §4 y §15 | `07-Plan-Sprint` de SelfHosted-Web. Los diez cortes verticales del Alcance 1 están declarados de forma cerrada **[E]** y ninguna de las dos figura entre ellos; ubicarlas exige decidir si abren un corte propio o se suman a uno existente |
