...
**Migración al conjunto normativo 4.0, 2026-07-28.** Este artefacto pasa a su nombre lógico estable, sin sufijo de versión, por la reformulación de D4 y D5; la versión 1.3 queda archivada en `SDD/Intake/_legacy/2026-07-28/`. El puntero al intake sigue al renombrado de ese documento, que pasó a la versión 2.0 con el mismo criterio. **El bloque de procedencia del framework de §1.1 del formato vigente no está declarado y no se completa a mano**: lo emite el orquestador al re-derivar el manifiesto, leyendo la entrada vigente del changelog del framework y la versión de cabecera de cada regla aplicada. Declararlo ahora afirmaría que la documentación existente se generó bajo el conjunto 4.0, que es falso, y D9 exige evidencia. Su ausencia es además lo que hace que la fase de reconciliación normativa clasifique a este destino en el caso «sin procedencia», que es el correcto.
