...
> Este documento captura qué quiere el cliente, cómo se compone la solución y cómo se construye cada proyecto de código.
...
Cada una se barre por su **predicado** —«determina», «se deduce de», «arrastra», «exige», «nace de», «necesita para existir»— y no por su sustantivo. R5 es la que esta pasada agregó tarde: se detectó como instancia suelta antes de promoverla a relación, y los dos restos que sobrevivieron a la primera corrección estaban exactamente ahí.
...
|---|---|---|
| V1 | «Proyecto» a secas designa **cualquiera de los dos sentidos**, y el lector desambigua por contexto | «Proyecto» a secas designa el **proyecto SelfHosted**, y sólo donde el contexto ya lo fijó; el otro sentido se escribe siempre **completo** |
...
| **Proyecto de código** | Cada una de las cuatro unidades de la composición de la solución declaradas en §13: `SelfHosted-Web`, `SelfHosted-Application`, `SelfHosted-Infrastructure` y `SelfHosted-Domain`. Es una unidad de compilación del repositorio, no algo que el usuario cree ni vea. Variante larga admitida: «proyecto de código fuente»; la forma canónica es la corta |
...
| Nombre de proyecto de código | `SelfHosted`, un **separador** y un **sufijo de rol**, sin la palabra «proyecto» delante. El separador tiene dos formas legítimas, según el perfil de convención de §13: **guion** en el `Nombre-Proyecto` y **punto** en el `nombre-proyecto-codigo` | `SelfHosted-Web` y `SelfHosted.Web`; `SelfHosted-Domain` y `SelfHosted.Domain` |

**El discriminador es el sufijo de rol, no el separador.** Los cuatro sufijos —`Web`, `Application`, `Infrastructure`, `Domain`— están presentes en las dos formas del nombre de código **sin excepción**, mientras que el término del producto no lleva ninguno. Por eso la regla es decidible por la forma de la palabra, sin depender del contexto, del formato ni de cuál de los dos separadores se haya usado. El monoespaciado se usa donde el nombre de código se cita como identificador, pero es una **ayuda de lectura, no el mecanismo**: la distinción se sostiene igual en texto plano. Se exceptúan las filas donde el nombre es el **valor de un campo declarado** —`Nombre-Proyecto` en la tabla de §13 y en las cuatro tablas de identidad de §17—, que quedan sin formato deliberadamente: §13 es la fuente del manifiesto derivado y su tabla no se toca, de modo que ponerle formato sólo a §17 haría que las dos declaraciones del mismo dato dejaran de coincidir.

**Prohibición que se deriva, y que un artefacto downstream debe respetar:** no se admite ninguna construcción que fusione los dos términos, del tipo «el proyecto SelfHosted-Web», «el proyecto SelfHosted.Web» o «el proyecto de código SelfHosted». La prohibición alcanza a las dos formas del separador. Cuando hay que nombrar una unidad de compilación se escribe su nombre solo —`SelfHosted-Web`— o «el proyecto de código `SelfHosted-Web`», nunca «el proyecto SelfHosted-Web». Es la única forma en que los dos términos colapsarían, y hoy no ocurre en ninguna de las quinientas ocurrencias del documento.
...
## §13 Proyectos de código de la solución

La composición se deriva directamente de la estructura de carpetas de `/src` declarada en `Analisis-Final-Integrado.md` §12 **[E]**, que define cuatro proyectos de código: dominio, aplicación, infraestructura y web. El despliegue es monolítico —un único proceso sirve la interfaz Blazor, la API REST y los servicios en segundo plano—, y por eso el proyecto de código principal es el web, del que los demás son dependencias internas.

Esta sección habla **exclusivamente de proyectos de código**, en el sentido que fija §12: unidades de compilación del repositorio. El otro sentido del término —el **proyecto SelfHosted**, que es lo que el usuario crea desde el portal— no aparece acá.
...
**Nota sobre los proyectos de prueba.** `SelfHosted.Domain.Tests`, `SelfHosted.Application.Tests` y `SelfHosted.Integration.Tests` **[E]** no se declaran como proyectos de código de la composición: son artefactos de la estrategia de testing de cada proyecto de código (§17 P.6) y viven bajo `/tests`, según el árbol de §16.
...
| Prefijo de paquetes redistribuibles | `Aplicada` | No se aplica en esta solución: ningún proyecto de código es redistribuible |
...
La solución aplica Clean Architecture con organización por módulos, en despliegue monolítico **[E]**. Como §13, esta sección habla de **proyectos de código**. La regla de dependencia es la del estilo: las dependencias apuntan hacia el dominio y nunca al revés.

| Proyecto de código | Qué expone a sus dependientes | Quién lo consume |
|---|---|---|
| SelfHosted-Domain | Entidades, objetos de valor, invariantes del modelo (I1 a I10, enumeradas en §17.4 P.2) y reglas de negocio (RN-01 a RN-34, transcriptas en el anexo [E-16](#2016--e-16--catálogo-de-reglas-de-negocio-rn-01-a-rn-34)) verificables sin infraestructura | Application, Infrastructure, Web |
...
Las cuatro fronteras corresponden a aristas de dependencia reales del grafo de §13. El punto de entrada del usuario final es `SelfHosted-Web`; el proyecto de código compartido por todos es `SelfHosted-Domain`.
...
El orden de construcción respeta el orden topológico de §13 dentro de cada etapa, no entre etapas: cada corte vertical toca los cuatro proyectos de código en el orden Domain → Application → Infrastructure → Web.
...
Cada proyecto de código de §13 tiene su carpeta en `/src` con su nombre de código. La estructura sigue las convenciones del ecosistema .NET: `src` y `tests` como raíces, un directorio por proyecto de código y espacios de nombres que replican la ruta. Ninguno es redistribuible, de modo que el prefijo de organización no se usa.
...
2. **Aislar la página del lienzo en `InteractiveAuto` o WebAssembly.** Es la mitigación M6 de RG-01 y se descarta salvo que PT-01 falle después de aplicar M1 a M4, porque cambia un requisito declarado del proyecto de código.
...
| Hacia otros proyectos de código | Consume `SelfHosted.Application` por referencia de proyecto de código, en proceso. Registra las implementaciones de `SelfHosted.Infrastructure` en el contenedor de dependencias durante el arranque |
...
La superficie de la API son los **treinta endpoints**, agrupados en las veintiocho filas transcriptas en el anexo [E-15](#2015--e-15--superficie-de-la-api-rest) **[E]**, cada uno con su ámbito: `proyectos:leer`, `proyectos:escribir`, `despliegues:ejecutar`, `catalogo:leer`, `catalogo:escribir` y `sistema:leer`. Las reglas que esos endpoints hacen cumplir, con su momento de validación y su código de respuesta, están en el anexo [E-16](#2016--e-16--catálogo-de-reglas-de-negocio-rn-01-a-rn-34).
...
Restricción heredada que este proyecto de código debe respetar **[E]**: el alcance del `DbContext` es **uno por operación**. Los servicios en segundo plano crean su propio alcance en cada ciclo y nunca comparten el de la interfaz.
...
**[S] S-02, confirmado el 2026-07-27**, en los umbrales numéricos; los niveles, los proyectos de código y el criterio de cierre son **[E]**.

| Nivel | Proporción de la pirámide | Proyecto de código | Framework | Qué cubre |
|---|---|---|---|---|
...
- Tests de contrato hacia otros proyectos de código: los casos de uso de `SelfHosted.Application` se ejercitan desde este proyecto de código sólo a través de sus interfaces públicas.
...
- **SemVer 2.0.0 y Conventional Commits, sin excepciones**, para toda la solución. La versión es única para los cuatro proyectos de código, porque se despliegan como un único artefacto.
...
| Nivel | Proporción | Proyecto de código | Framework | Qué cubre |
|---|---|---|---|---|
...
Los mismos stages y quality gates de §17.1.P.8, ejecutados sobre la solución completa por `scripts/build.sh` y `scripts/test.sh`. La cobertura mínima que se verifica para este proyecto de código es la de su P.6: 80 % de líneas y 70 % de ramas.
...
|---|---|---|
| Toda salida al mundo exterior pasa por una abstracción declarada en este proyecto de código | Que la capa web hable directamente con la infraestructura | Es la condición que hace verificable la regla de aislamiento del cliente del motor **[E]** |
...
1. **Un proyecto de código por adaptador** (uno de persistencia, uno de contenedores, uno de sistema): multiplicaría las unidades de compilación sin ganar aislamiento real, porque todas se despliegan juntas y ninguna se publica por separado.
2. **Adaptadores dentro del proyecto de código web**: la regla de aislamiento del cliente de Docker dejaría de ser verificable por compilación, que es exactamente lo que se quiere evitar.
...
Modelo de datos: el esquema relacional está transcripto completo en el anexo [E-9](#209--e-9--esquema-relacional-de-la-base-sqlite). Tres decisiones de esquema que este proyecto de código debe sostener **[E]**:
...
Tests de contrato: este proyecto de código debe pasar la batería de contrato que `SelfHosted.Application` define para cada abstracción. La verificación de PT-02 —listar contenedores del host, crear, arrancar, detener y eliminar un contenedor de prueba desde código, construir una imagen desde un Dockerfile con contexto en el directorio de datos, y alcanzar por red el contenedor creado— se materializa como prueba de integración automatizada, no como comprobación manual **[E]**.
...
Los mismos stages y quality gates de §17.1.P.8. Particularidades de este proyecto de código:
...
| Nivel | Proporción | Proyecto de código | Framework | Qué cubre |
|---|---|---|---|---|
| Unitarias | 100 % | `SelfHosted.Domain.Tests` | xUnit | Invariantes del modelo (I1 a I10) y reglas de negocio (RN-01 a RN-34), **sin infraestructura** **[E]**, en particular la regla de conflicto de direcciones IP |
...
Los cincuenta y ocho casos del anexo [E-22](#2022--e-22--casos-de-prueba-derivados-de-las-configuraciones-reales) son el punto de partida de esta batería: cada uno lleva su entrada concreta y su resultado esperado, y varios usan como dato de entrada las configuraciones reales del anexo [E-20](#2020--e-20--configuraciones-reales-de-contenedor-ofuscadas), de modo que las pruebas se escriben contra formas que ya se sabe que existen en un servidor real. Las treinta y cuatro reglas del anexo [E-16](#2016--e-16--catálogo-de-reglas-de-negocio-rn-01-a-rn-34) se traducen cada una en al menos una prueba **[E]**: la fuente declara ese catálogo pensado exactamente para eso, con el momento de validación y la respuesta esperada de cada regla, que son el enunciado y la aserción de la prueba. Cada etapa que introduzca una regla nueva debe traer su prueba: es el criterio de cierre de etapa.
...
- Las pruebas de este proyecto de código **no pueden requerir el socket del motor de contenedores ni la base de datos**: si lo hicieran, la separación de capas estaría rota. Es un gate de arquitectura.
...
| Trazabilidad de las reglas | Cada regla RN-01 a RN-34 identificable en el código por su identificador **[D]** |
...
```json
{
  ...
  "variables": [
    ...
    { "clave": "DB_USER", "valor": "portal", "secreta": false, "origen": "referencia", "referencia": "${{ db#103.POSTGRES_USER }}", "resueltaEn": "2026-07-26T09:02:09-03:00" },
...
| **Forma vinculada** | `${{ db#103.SELFHOSTED_HOST }}` | Lo que se persiste en `variables.referencia`. El `#103` es el identificador del servicio destino y es **el vínculo**; el `db` que lo precede es el nombre renderizado, que el sistema mantiene al día y que no significa nada para el resolutor |
...
- **Al renombrar un servicio**, no cambia ningún vínculo: sólo se refresca el nombre renderizado de las expresiones que lo apuntan, que se localizan por las aristas, que ya guardan `destino_servicio_id`. Ninguna referencia se rompe (RN-33).
...
### §20.16 · E-16 · Catálogo de reglas de negocio RN-01 a RN-34
...
|---|---|---|---|
| RN-01 | El nombre de servicio es único dentro del proyecto SelfHosted, en minúsculas, con guiones, de 1 a 32 caracteres | Alta y edición | `422` con el campo señalado |
...
| RN-33 (enunciado **[D]**, forma **[D-i]**) | **[D], D-8:** renombrar un servicio **no invalida ni altera ninguna referencia** que le apunte, y no produce cambios pendientes en el changeset. Un servicio puede llamarse `shared` sin que ninguna expresión quede ambigua. **[D-i]:** se logra persistiendo la expresión en su forma vinculada, con el identificador del servicio destino, y renderizando el nombre al mostrarla (anexo E-4) | Renombrado de un servicio | Invariante, verificable por prueba: tras renombrar, toda expresión que apuntaba al servicio sigue resolviendo al mismo valor y la interfaz muestra el nombre nuevo |
...
| Catálogo completo de reglas verificables RN-01 a RN-34, con momento de validación y respuesta | E-16 |
...
- [x] §13 enumera los 4 proyectos de código, cada uno con uno de los 8 valores D8, señala el principal (SelfHosted-Web) y el grafo de dependencias es acíclico.
...
- [x] §17 está completo para cada uno de los 4 proyectos de código de §13: identidad más P.1 a P.12.
...
- [x] Los insumos de generación de pruebas (reglas RN-01 a RN-34 con momento de validación y respuesta, invariantes I1 a I10, máquina de estados, contratos de la API y esquema de datos) están transcriptos en E-16, E-17, E-15, E-9 y §17.4 P.2.
...
| §14 estilo de solución | `Solucion/` y `05-Arquitectura-Tecnica/` de cada proyecto de código | Vista de solución, grafo de dependencias |
...
| §17 P.x por proyecto de código | `05`, `08`, `09`, `00` de cada proyecto de código | ADR (DA-01 a DA-08), estrategia de testing, pipeline, NFR |
...
| §18 samples y puertas técnicas | `10-Examples/` de cada proyecto de código | Ejemplo-XX; SM-01 y SM-02 son las materializaciones de PT-01 y PT-02 |
...
| E-16 reglas RN-01 a RN-34 | `02-Especificacion-Funcional/` y `08-Calidad-Y-Pruebas/` | RN-XX y casos de prueba, uno por regla |
...
| E-22 casos de prueba | `08-Calidad-Y-Pruebas/` de cada proyecto de código | Casos de prueba con entrada y resultado esperado, uno o más por regla |
...
| 1.2 | 2026-07-28 | Actualización de §14: la fila de SelfHosted-Domain pasa de declarar RN-01 a RN-20 a declarar RN-01 a RN-34. Motivo: el catálogo de reglas incorporó once reglas nuevas. Origina el cambio: D-1, D-2, D-5, D-6 y D-7. | Orquestador SDD |
...
| 1.2 | 2026-07-28 | Actualización de §17.1 P.3: la superficie de la API pasa de veintidós a veintiséis endpoints y la cita del catálogo de reglas pasa a RN-01 a RN-34. Motivo: las decisiones exigieron cuatro endpoints nuevos, transcriptos en E-15. Origina el cambio: D-1, D-2, D-5 y D-7. | Orquestador SDD |
...
| 1.2 | 2026-07-28 | Actualización de §17.4 P.6: la estrategia de testing del dominio pasa a citar RN-01 a RN-34 y el recuento de casos de E-22. Motivo: el catálogo de reglas y el de casos de prueba se ampliaron. Origina el cambio: D-1, D-2, D-5, D-6 y D-7. | Orquestador SDD |
| 1.2 | 2026-07-28 | Actualización de §17.4 P.10: la fila de trazabilidad de las reglas pasa a citar RN-01 a RN-34. Motivo: coherencia con el catálogo ampliado. Origina el cambio: D-1, D-2, D-5, D-6 y D-7. | Orquestador SDD |
...
| 1.2 | 2026-07-28 | Actualización de §21: se agregan nueve filas a la matriz de cobertura, una por cada capacidad o regla nueva, con los escenarios y casos de prueba que la ejercitan, y se ajusta la fila del catálogo de reglas a RN-01 a RN-34. Origina el cambio: las siete decisiones. | Orquestador SDD |
...
| 1.2 | 2026-07-28 | Actualización de «Trazabilidad downstream»: la fila de E-16 pasa de RN-01 a RN-20 a RN-01 a RN-34. Motivo: coherencia con el catálogo ampliado. Origina el cambio: D-1, D-2, D-5, D-6 y D-7. | Orquestador SDD |
...
| 1.2 | 2026-07-28 | Actualización de §14, §17.1 P.3, §17.3 P.5, §17.4 P.6, §17.4 P.10, §21 y «Trazabilidad downstream»: las citas al catálogo de reglas pasan de RN-01 a RN-31 a RN-01 a RN-34, el recuento de casos de prueba de 49 a 54, y la matriz de cobertura reemplaza la fila de la arista con dos orígenes por tres filas —mecanismo único, variables provistas y referencia de red frente a referencia de dato—. Origina el cambio: D-6, segunda pasada. | Orquestador SDD |
