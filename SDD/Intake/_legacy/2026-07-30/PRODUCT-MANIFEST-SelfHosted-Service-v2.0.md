...
| Versión | 2.0 | — |
| Fecha | 2026-07-30 | — |
| Estado | **En revisión.** Re-derivado en la fase M3 de la migración normativa 4.1 → 6.0 el 2026-07-30, desde §13 del intake migrado. Queda en revisión hasta la confirmación explícita del agente humano del proyecto, que es la detención que `Master-Prompt.md` §3 paso 3 declara | — |
...
| `PRODUCT-INTAKE-template` | **2.1** — ver la nota de estado mixto |
| `PRODUCT-MANIFEST-template` | **4.1** — ver la nota de estado mixto |

**Nota de estado mixto, incorporada en la versión 2.0 (fase M3 de la migración normativa 4.1 → 6.0).** Este bloque declara **dos cosas que hoy son distintas**, y la distinción es deliberada.

Las catorce filas de arriba —el conjunto, el master-prompt y las reglas— siguen declarando el **conjunto 4.1**, que es la normativa bajo la que `SDD/Docs/` está efectivamente generada. `Master-Prompt-Migracion.md` §7 paso 2 lo obliga: la procedencia **no se toca en M3**, porque adelantarla produciría una afirmación falsa mientras los 141 documentos de `SDD/Docs/` siguen sin migrar. Su actualización a 6.0 es trabajo de la fase M5 y **solo ocurre si toda la cadena queda migrada**; con migración parcial la procedencia se queda como está, que es lo que sigue siendo cierto (`Migracion-Rules.md` §4.6).

Las dos filas nuevas de plantilla, en cambio, declaran **2.1 y 4.1, que son las vigentes**, y no las del origen. No es una inconsistencia: son afirmaciones sobre **los dos documentos de entrada**, no sobre `SDD/Docs/`, y los dos ya fueron reexpresados. El intake se migró a `PRODUCT-INTAKE-template` 2.1 en la fase M2 el 2026-07-30 y este manifiesto está estructurado bajo `PRODUCT-MANIFEST-template` 4.1 desde esta misma versión. Declarar acá las versiones de origen sería falso sobre dos archivos que acaban de reescribirse, y D9 lo prohíbe igual que prohíbe adelantar las otras catorce.

Las dos filas son exactamente la instrumentación que la entrada `[6.0]` del `CHANGELOG.md` del framework agregó, y por cuya ausencia este manifiesto **dejaba de cumplir**: sin ellas, un cambio de estructura de plantilla no movía ninguna versión declarada y era invisible para el diff normativo.

**Qué falta para cerrar la procedencia.** Que M4 migre los 141 documentos de `SDD/Docs/` y que M5 verifique que ninguna fila del plan quedó sin resolver. Recién entonces las catorce filas pasan a 6.0 y se suma `Migracion-Rules` 1.0 a las transversales aplicadas.
