...
|---|---|
| Las cinco exclusiones `F-18` a `F-22` del `Alcance-Proyecto.md` | **Derivables, no originadas.** El intake §4 las declara `Won't Have v1` —etiqueta MoSCoW con la que el Product Owner declara la exclusión— y §9 trae las exclusiones correspondientes, las siete marcadas `[E]`. AG-00 las formalizó, que es exactamente su trabajo bajo 2.1. No hubo nada que elevar |
