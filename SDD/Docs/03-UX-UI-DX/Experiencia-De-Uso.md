...
| Tablero de estado | Sin proyectos SelfHosted | Esqueleto de las tres capas | Tres capas con sus barras | Lectura del estado del servidor no disponible | — | Dato con antigüedad declarada; **sugerencia de limpieza vigente**, y su ausencia que **no se representa** |
...
| Configuración del sistema | No aplica: siempre hay rango y retención | Esqueleto de campos | **Seis** secciones de configuración | Error inline por campo | Confirmación | Credencial mostrada una única vez; **umbral de la sugerencia sin valor por defecto declarado** |
...
**Actualización de la versión 2.2, 2026-07-30.** **`B-UX-30` se cierra**: era trabajo propio de esta categoría y no una decisión a elevar, y la propagación se aplicó a las cuatro superficies que enumeraba. Se verificó una por una que la propagación las alcanzara de verdad, y a las cuatro las alcanza: `SUP-09` y `SUP-12` **ganan contenido** —la ubicación secundaria de la sugerencia y los dos descriptores de su umbral—, y `SUP-06` y `SUP-11` **tenían una brecha declarada abierta que dejó de estarlo** —`B-UX-26` y `B-UX-23`—. Ninguna quedó fuera y ninguna se retiró de la lista por no corresponder. **No se abre ninguna brecha nueva.** El resultado son **veinticinco brechas vigentes sobre treinta identificadores emitidos**: `B-UX-15` retirada, y `B-UX-09`, `B-UX-23`, `B-UX-26` y `B-UX-30` cerradas.
...
| B-UX-30 | ~~**Propagación pendiente a cuatro wireframes hermanos que esta ronda no alcanzó.**~~ · **CERRADA el 2026-07-30.** Las cuatro superficies se verificaron una por una y la propagación **alcanza a las cuatro**: `SUP-09` 2.1 declara la ubicación secundaria de la sugerencia con su §3.2, sus dos estados y la prohibición de confirmar desde ahí; `SUP-12` 2.1 emite la sexta sección con los **dos descriptores del umbral** y sus dos estados; `SUP-06` 2.1 cierra `B-UX-26` en su §3.5 y declara la consecuencia y el límite de que `Q-15` rija hacia adelante; y `SUP-11` 2.1 cierra `B-UX-23` en su §3.8, cuyo estado vacío pasa a declarar cuatro cosas. Enunciado original, conservado: El despacho de esta ronda acota el trabajo a `SUP-17`, `SUP-18`, `SUP-19`, este marco, el glosario y el índice, y hay cuatro superficies vigentes cuyo contenido queda desactualizado y **no se toca desde acá**: `SUP-06`, panel lateral del servicio, cuyo §3.5 cita `B-UX-26` como abierta cuando ya está cerrada; `SUP-09`, tablero de estado, que pasa a alojar la **ubicación secundaria** de la sugerencia de limpieza especificada en `SUP-18` §3.5; `SUP-12`, configuración del sistema, que pasa a alojar los **descriptores del umbral** de esa misma sugerencia; y `SUP-11`, catálogo de plantillas, cuyo §3.8 cita `B-UX-23` como abierta. Se declara en lugar de aplicarse en silencio, y en lugar de dejarlo sin registro | [`Wireframes-Panel-Lateral-Del-Servicio.md`](Wireframes/Wireframes-Panel-Lateral-Del-Servicio.md) §3.5; [`Wireframes-Tablero-De-Estado.md`](Wireframes/Wireframes-Tablero-De-Estado.md); [`Wireframes-Configuracion-Del-Sistema.md`](Wireframes/Wireframes-Configuracion-Del-Sistema.md); [`Wireframes-Catalogo-De-Plantillas.md`](Wireframes/Wireframes-Catalogo-De-Plantillas.md) §3.8 | Cerrada. Era trabajo de esta categoría, no una decisión a elevar | **Cerrada el 2026-07-30.** La fila se conserva porque otros artefactos citan el identificador |
...
| --- | --- | --- |
| 2.3 | 2026-07-30 | **Corrección derivada del hallazgo `P2` del informe de audit [`B2-Retroalimentacion-Decisiones-2026-07-30-r1.md`](../Audit/B2-Retroalimentacion-Decisiones-2026-07-30-r1.md)**, veredicto APROBADO CON OBSERVACIONES, cero P0. El hallazgo era contra el `README.md` —una cifra de reparto de versiones que no se sostenía—, y al recontar en disco todas las cifras autorreferenciales de la categoría apareció **una de este documento** que tampoco se sostenía. Sube **minor**: ninguna sección se renumera, ninguna superficie, flujo, estado, brecha, contradicción ni supuesto cambia, y ninguna decisión abierta se cierra. **Lo corregido, uno solo:** §9.1 decía «Instancia la tabla tipo de `Rules-UX-UI-DX.md` §4.3 con sus catorce filas», y la afirmación era ambigua y se leía como si la tabla de §9.1 tuviera catorce. La tabla tipo de la regla tiene catorce; **la de §9.1 tiene dieciséis**, porque esta categoría le suma dos filas propias —representaciones asociadas, y anchos de verificación del comportamiento responsivo—. Se declara la composición en lugar de la cifra sola. **Recuento completo verificado en disco, y qué sí se sostuvo**: las **19** filas del mapa de estados de §4.1, las **19** superficies de §9.2, los **39** casos de uso de §9.3 —contiguos de `CU-01` a `CU-39`, sin faltantes—, y los **30** identificadores de brecha de §10.2 con **25** vigentes, que es exactamente lo que §10.2 declara. Ninguna de ésas se tocó. **Ningún artefacto fuera de `03-UX-UI-DX` se tocó.** La versión 2.2 queda archivada en [`_legacy/2026-07-30/Experiencia-De-Uso-v2.2.md`](_legacy/2026-07-30/Experiencia-De-Uso-v2.2.md), con su bloque de archivado antepuesto y su cuerpo sin modificar. Ninguna fila anterior de este control de cambios se reescribió (`SDD-Development-Guide.md` §VI.2) |
| 2.2 | 2026-07-30 | **Cierre de la brecha `B-UX-30`: propagación de la retroalimentación del paso 6 de la Fase B2 a las cuatro superficies que la versión 2.1 no había alcanzado.** Sube **minor**: ninguna sección se renumera, ningún flujo, principio, heurística, criterio de accesibilidad, umbral de performance percibida ni supuesto de derivación cambia de contenido, y la tabla de superficies de §9.2 sigue en **diecinueve**. **Se verificó una por una si la propagación alcanzaba a las cuatro, y alcanza a las cuatro**, en lugar de darlo por sentado: `SUP-09` y `SUP-12` **ganan contenido** por la decisión `Q-17` —la ubicación secundaria de la sugerencia de limpieza y los dos descriptores de su umbral, que el intake v3.2 delega en esta categoría—, y `SUP-06` y `SUP-11` **tenían una brecha declarada abierta que dejó de estarlo** por `Q-15` y `Q-27`. Ninguna se retiró de la lista por no corresponder. **§4.1** actualiza las dos filas alcanzadas del mapa de estados: la del tablero suma la sugerencia vigente y su ausencia que **no se representa**, y la de la configuración del sistema suma el umbral **sin valor por defecto declarado** y pasa a **seis** secciones. **§10.2 pasa de veintiséis brechas vigentes a veinticinco, sobre los mismos treinta identificadores**: **`B-UX-30` se cierra** con el detalle de qué quedó declarado en cada una de las cuatro, y **no se abre ninguna brecha nueva**. La fila conserva su identificador y su enunciado original tachado, con el criterio ya aplicado a `B-UX-09`, `B-UX-15`, `B-UX-23` y `B-UX-26`. **Ninguna decisión abierta se cerró y ningún valor plausible se completó**: `B-UX-28` sigue abierta y `SUP-12` dibuja los dos campos del umbral **con su descriptor y sin valor por defecto**, que es distinto de dibujarlos con un número. Siguen abiertas `Q-16`, `Q-18`, `Q-19`, `Q-20`, `Q-21` y `Q-28`. **Ningún artefacto fuera de `03-UX-UI-DX` se tocó.** La versión 2.1 queda archivada en [`_legacy/2026-07-30/Experiencia-De-Uso-v2.1.md`](_legacy/2026-07-30/Experiencia-De-Uso-v2.1.md), con su bloque de archivado antepuesto y su cuerpo sin modificar. Ninguna fila anterior de este control de cambios se reescribió (`SDD-Development-Guide.md` §VI.2) |
