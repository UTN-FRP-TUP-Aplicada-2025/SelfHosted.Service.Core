...
Hay un segundo consumidor de la solución, el **automatismo de integración continua** (`Vision-Producto.md` §2.1), pero su superficie es la API REST con una credencial de máquina, no una pantalla. Por lo tanto:
...
| Capa del catálogo | Documento | Por qué aplica en esta solución |
| --- | --- | --- |
| Base, siempre | `Design-Rules-Web-Generico.md` 1.2 | Piso obligatorio de todo proyecto con interfaz web |
...
El único token que esta solución necesita y el catálogo no tiene es el del estado «pendiente de aplicar». Se declara como brecha `B-UX-05` en §10.2 y como contradicción en §10.1, y no se resuelve acá inventando un valor.
...
| Correspondencia entre el sistema y el mundo real | Vocabulario del glosario del dominio del cliente en toda la interfaz | Intake §12; `Vision-Producto.md` §9 | Revisión de microcopy contra el glosario, con `Glosario-UX.md` como puente |
...
| Jakob | Un mismo concepto se ve y se comporta igual en toda la solución | El estado de un servicio se lee igual en el nodo del lienzo, en la lista del tablero y en la cabecera del panel lateral: mismo par de insignia y etiqueta, misma correspondencia de estado |
...
| Campo del contrato | Valor en esta solución | Fuente |
| --- | --- | --- |
...
| Prefijo de nombre reservado de los contenedores que la solución gobierna | Sin clasificar por ninguna fuente. El intake lo declara «reservado y configurable, distinto en desarrollo y en producción» sin decir quién lo fija | Intake §17.P.5 | **No se dibuja** hasta que se resuelva la brecha `B-UX-06` |

Un descriptor puede existir sin superficie: los parámetros de entorno conservan su contrato y no se renderizan. Y cuando un parámetro de entorno condiciona el efecto de una superficie de aplicación, la superficie lo declara como información y no ofrece cambiarlo. El caso concreto de esta solución es el rango de direcciones: el intake declara que el rango de desarrollo debe ser distinto del de producción y sin solapamiento (§17.P.5), lo que condiciona los valores admisibles del rango gestionado sin que la superficie pueda gobernar esa separación.
...
**Origen:** intake §6, flujo 1, transcripto paso por paso en el anexo E-10. **Casos de uso:** CU-01, CU-16, CU-13, CU-04, CU-03, CU-22, CU-24, CU-18. **Superficies:** Listado de proyectos → Lienzo del proyecto → Panel lateral del servicio → Cajón de cambios pendientes.
...
3. `+ Nuevo servicio` → **Imagen de registro** → el nodo aparece, también pendiente.
...
**Origen:** NB-07; anexo E-18, tablero del segundo alcance. **Casos de uso:** CU-26, CU-27, CU-28. **Superficies:** Tablero de estado → Lienzo del proyecto → Registro del contenedor.
...
**Salida.** El consumo queda atribuido a un servicio concreto.
...
`Design-Rules-Config-Esquema.md` §6 exige que toda propuesta de cambio se previsualice antes de aplicar y se confirme explícitamente, y que la interfaz nunca aplique directo. En esta solución esa frontera **ya existe con nombre propio en el intake**: es el conjunto de cambios pendientes con su informe de impacto, declarado como el mecanismo de edición transaccional del proyecto SelfHosted.

| Requisito de la frontera, `Design-Rules-Config-Esquema.md` §6 | Realización en esta solución | Fuente |
| --- | --- | --- |
...
Las dos ubicaciones obligatorias en esta solución:
...
| Advertencia que no bloquea | Higiene del registro: variable compartida huérfana, nombre repetido en el mismo ámbito, clave que ya existe al instanciar, referencia sin uso | Se informa, nunca se impide. La decisión se toma con la información delante y es reversible | RN-37; intake §4, nota sobre F-25 |
...
| --- | --- |
| Persona objetivo | Administrador único de la solución, identificado por rol: [`Vision-Producto.md`](../00-Contexto/Vision-Producto.md) §2.1 y §2.2 |
| CU origen | Los 36 casos de uso de [`02-Especificacion-Funcional`](../02-Especificacion-Funcional/Especificacion-Funcional.md) §3, con interacción humana en 35 de ellos; ver §9.3 |
...
| Wireframes asociados | Los 16 de [`Wireframes/`](Wireframes/); ver §9.2 |
| Representaciones asociadas | Las 4 de [`Representaciones/`](Representaciones/) |
| US a generar en 06 | Las 118 historias de usuario provisionales de la matriz de [`Especificacion-Funcional.md`](../02-Especificacion-Funcional/Especificacion-Funcional.md) §6, con la numeración definitiva como potestad de `06-Backlog-Tecnico` (brecha B-19 de esa categoría) |
...
**Dieciocho** superficies. Las once primeras corresponden a nodos del mapa de navegación del anexo E-18; las siete últimas declaran de dónde salen, porque E-18 no las maqueta.
...
| SUP-18 | Imágenes | [`Wireframes-Imagenes.md`](Wireframes/Wireframes-Imagenes.md) | `/imagenes` | CU-37, CU-38 | **E-18 no la maqueta.** Derivada del anexo E-23, que es nuevo en el intake v2.4. **Depende de decisiones abiertas y lo declara** |

El mínimo que la tabla de adaptabilidad fija para `web-monolith` es de cuatro superficies clave. E-18 declara más, y la cobertura de los casos de uso con interacción humana relevante lleva la cuenta a **dieciocho**.
...
**`SUP-18`, imágenes, es una superficie que no existía porque su dominio no existía.** El ciclo de vida de las imágenes no estaba en el intake antes de su versión 2.4. **Depende de cinco decisiones abiertas** y su wireframe las declara una por una en su §5.1, con la consecuencia de que la maqueta **no debe construirla** hasta que `Q-15` y `Q-17` se cierren.
...
Los **38** casos de uso de `02-Especificacion-Funcional`, con la superficie que los materializa. Ningún caso de uso con interacción humana queda sin superficie.

| CU | Superficies | CU | Superficies |
| --- | --- | --- | --- |
| CU-01 | SUP-04 | CU-19 | SUP-06, SUP-12 |
| CU-02 | SUP-04 | CU-20 | SUP-14 |
| CU-03 | SUP-05, SUP-06, **SUP-17** | CU-21 | SUP-14 |
...
`Rules-UX-UI-DX.md` §1.4 obliga a «reservar la ranura del asistente de IA (forward-compat) sin construirla» en todo proyecto que cargue la extensión de configuración por esquema, y `Design-Rules-Config-Esquema.md` §4.7 la especifica. Ninguna fuente de esta solución declara asistencia de un modelo de lenguaje: el indicador correspondiente es falso y el alcance de §4 del intake no la enumera ni entre lo pospuesto. Además, `Design-Rules-Acceso-Monousuario.md` §10 declara como anti-patrón «mostrar deshabilitado lo que no aplica».
...
| B-UX-05 | El estado «pendiente de aplicar» no tiene token en el catálogo base y el catálogo prohíbe definirlo por proyecto. Ver `C-UX-01` | §4.2 de este documento | Mantenedor del catálogo de diseño (AG-ROOT) | Confirmada |
...
| B-UX-23 | **El primer minuto de uso no tiene camino para quien no sabe una dirección de imagen.** El catálogo arranca vacío porque el producto no trae contenido de fábrica —decisión D-16— y el producto **no declara si existe alguna forma de explorar un registro de imágenes**. Es la pendiente `Q-27` del intake §19. Su respuesta decide si hay **una superficie nueva** o **una línea de ayuda**, que es una diferencia grande de trabajo, y por eso no se presume | [`Wireframes-Alta-De-Servicio.md`](Wireframes/Wireframes-Alta-De-Servicio.md) §3.6; [`Wireframes-Catalogo-De-Plantillas.md`](Wireframes/Wireframes-Catalogo-De-Plantillas.md) §3.8 | Agente humano del proyecto | **Nueva en la versión 1.1.** Origen: §22.5 y §23.3 del documento de trabajo `Redefinicion-Servicio.md` v2.0 |
...
| B-UX-26 | **El digesto de la imagen en uso depende de que alguien lo registre.** El panel lateral y el inventario de imágenes lo necesitan para responder qué corre exactamente, y el registro del digesto por despliegue es la pendiente `Q-15` del intake §19, abierta. **Sin ella los dos componentes no tienen qué mostrar** | [`Wireframes-Panel-Lateral-Del-Servicio.md`](Wireframes/Wireframes-Panel-Lateral-Del-Servicio.md) §3.5; [`Wireframes-Imagenes.md`](Wireframes/Wireframes-Imagenes.md) §5.1 | Agente humano del proyecto | **Nueva en la versión 1.1** |
| B-UX-27 | **Cinco tramos de la superficie de imágenes dependen de decisiones abiertas**, `Q-15` a `Q-21`: el indicador de uso, la separación entre lo administrado y lo ajeno, el modo de disparo de la limpieza, el alcance de la marca de conservada, y el ámbito de credencial de la limpieza por API. El wireframe los enumera uno por uno y **no completa ninguno**. Consecuencia declarada: **la maqueta no debe construir `SUP-18`** hasta que `Q-15` y `Q-17` se cierren | [`Wireframes-Imagenes.md`](Wireframes/Wireframes-Imagenes.md) §5.1 | Agente humano del proyecto | **Nueva en la versión 1.1** |
