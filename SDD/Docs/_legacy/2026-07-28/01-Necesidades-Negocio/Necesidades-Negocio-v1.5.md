...
| Documento | Necesidades-Negocio-v1.5.md |
| Versión | 1.5 |
| Estado | Propuesto |
| Fecha | 2026-07-28 |
| Autor | Analista de Negocio Senior (AG-01) |
| Cantidad de NB | 8 |
| Versión del catálogo de NB | 1.5 |
...
| Trazabilidad downstream | CU-01 a CU-36 (previstas en 02-Especificacion-Funcional); 06-Backlog-Tecnico; 07-Plan-Sprint; 08-Calidad-Y-Pruebas |
...
|---|---|---|---|---|---|
| NB-01 | Visibilidad unificada de la arquitectura de un conjunto de servicios | Must Have | CU-01 a CU-05 | Propuesto | [NB-01](Necesidades-De-Negocio/NB-01-Visibilidad-Unificada-De-La-Arquitectura-v1.5.md) |
| NB-02 | Adoptabilidad del parque existente sin reinstanciar lo que ya está en producción | Must Have | CU-06 a CU-08 | Propuesto | [NB-02](Necesidades-De-Negocio/NB-02-Adopcion-Del-Parque-Existente-v1.2.md) |
| NB-03 | Reproducibilidad de la arquitectura ante la pérdida del servidor | Should Have | CU-09 a CU-12 | Propuesto | [NB-03](Necesidades-De-Negocio/NB-03-Reproducibilidad-De-La-Arquitectura-v1.2.md) |
| NB-04 | El alta de un servicio deja de ser un ejercicio de copiar y adaptar | Must Have | CU-13 a CU-17 | Propuesto | [NB-04](Necesidades-De-Negocio/NB-04-Alta-De-Servicios-Sin-Copiar-Y-Adaptar-v1.2.md) |
| NB-05 | Arranque previsible: orden deducido y conflictos de dirección detectados antes de fallar | Must Have | CU-18 a CU-21 | Propuesto | [NB-05](Necesidades-De-Negocio/NB-05-Arranque-Sin-Conflictos-De-Direccion-v1.4.md) |
| NB-06 | Cambios de configuración revisados antes de aplicarse y aplicados en un solo lote | Must Have | CU-22 a CU-25 | Propuesto | [NB-06](Necesidades-De-Negocio/NB-06-Cambios-Revisados-Y-Aplicados-En-Lote-v1.4.md) |
| NB-07 | Atribución del consumo del servidor a un servicio concreto | Should Have | CU-26 a CU-28 | Propuesto | [NB-07](Necesidades-De-Negocio/NB-07-Atribucion-Del-Consumo-De-Recursos-v1.2.md) |
| NB-08 | Control de acceso al panel que gobierna el host y credenciales de máquina acotadas | Must Have | CU-29 a CU-33 | Propuesto | [NB-08](Necesidades-De-Negocio/NB-08-Control-De-Acceso-Y-Credenciales-De-Maquina-v1.2.md) |
...
Cada capacidad de SOLUTION-INTAKE §4 tiene exactamente una NB responsable. Las capacidades F-18 a F-22 están excluidas de la primera versión y no generan necesidad. F-23, F-24 y F-25 se incorporaron al intake el 2026-07-28 por las decisiones D-5, D-6 y D-13, y su asignación está argumentada en §6 de este índice.

| Capacidad | Prioridad declarada | NB responsable |
|---|---|---|
| F-01 alta del administrador único y sesión | Must Have | NB-08 |
| F-02 proyectos SelfHosted con su modo de red y su persistencia | Must Have | NB-01 |
...
| F-25 higiene del modelo | Could Have | NB-01 |

### 4.2 De NB a casos de uso previstos

Los casos de uso se generan por proyecto de código en 02-Especificacion-Funcional. La numeración de CU-01 a CU-36 es única en toda la solución y no colisiona entre necesidades. Treinta y uno corresponden a `SelfHosted-Web`, que es donde viven la superficie de usuario y la interfaz para automatismos; las cinco restantes corresponden a las librerías, en los puntos donde el caso de uso es verificable sin la superficie web. Los identificadores CU-34, CU-35 y CU-36 se agregaron el 2026-07-28 al final de la numeración, sin renumerar los anteriores, para no invalidar ninguna referencia ya emitida.

| NB | CU previstas | Proyecto de código | Estado |
|---|---|---|---|
| NB-01 | CU-01, CU-02, CU-03, CU-04, CU-05, CU-36 | SelfHosted-Web | a generar |
...
Reparto por proyecto de código: 31 casos de uso en `SelfHosted-Web`, 3 en `SelfHosted-Infrastructure`, 1 en `SelfHosted-Application` y 1 en `SelfHosted-Domain`. La categoría 02 puede desdoblar o reagrupar, siempre que conserve la trazabilidad a la NB de origen y no reutilice un identificador ya asignado.

Extensión declarada de la Tabla C. La Tabla C de `Rules-Necesidades-Negocio.md` §4.4 fija tres columnas: `NB`, `CU prevista` y `Estado`. Las ocho §7 de este catálogo y la tabla anterior agregan una cuarta columna, `Proyecto de código`, sin quitar ninguna de las tres originales. La extensión es deliberada y responde a una condición de esta solución: los casos de uso se generan por proyecto de código y la composición tiene cuatro, de modo que sin esa columna la categoría 02 no sabría a qué proyecto de código despachar cada caso de uso. Queda registrada aquí para que no se lea como desvío del formato; si el orquestador prefiere el formato estricto de tres columnas, la información se reubica dentro de la celda de la CU sin pérdida.
...
| CU | 02-Especificacion-Funcional, CU-01 a CU-36 | a generar |
...
Las ocho necesidades suman cuarenta y cuatro criterios de éxito. Cuarenta y dos toman su número de una métrica de negocio declarada, de un umbral no funcional declarado o del dimensionamiento verificado del intake. Los dos restantes son derivaciones de este catálogo, están marcados `[D]` en la tabla de la necesidad correspondiente y en la nota al pie de su §5, y requieren confirmación explícita del cliente antes de tratarse como cerrados.
...
| F-25 se ubica en NB-01, y no abre necesidad propia | Se evaluó abrirle una necesidad, porque su dolor no es no ver la arquitectura sino que se degrade sin que nadie lo note, y ese matiz es real. Se descartó por tres razones. La primera es que el público y la métrica son los mismos que los de NB-01 —el administrador único, y la confianza en que lo declarado sigue siendo cierto—, y la regla de partición pide públicos y métricas distintos. La segunda es que F-25 presupone por completo a NB-01: sin registro de proyectos SelfHosted, servicios y relaciones no hay nada cuya higiene revisar, de modo que una necesidad propia sería un nodo que depende enteramente de otra y no aporta recorte. La tercera es de tamaño: una necesidad con una sola capacidad Could Have, estrictamente informativa y de la que ningún flujo de usuario depende, sería la más delgada del catálogo y caería en el recorte demasiado granular que las reglas señalan como defecto. Se incorpora entonces a NB-01 como la parte que sostiene su promesa a lo largo del tiempo: el registro no sólo tiene que ser cierto el primer día |
...
| 1.5 | 2026-07-28 | Propagación de la quinta y última pasada sobre el intake. Las capacidades cubiertas pasan de diecinueve a veinte con el alta de F-25, higiene del modelo, asignada a NB-01 con su argumento y con el descarte de abrirle necesidad propia registrado en §6. Los casos de uso pasan de treinta y cinco a treinta y seis y los criterios de éxito de cuarenta y tres a cuarenta y cuatro, con las dos derivaciones `[D]` sin cambios. El principio de identidad de objeto amplía el séptimo criterio de NB-01, que pasa a cubrir el renombrado de cualquier elemento y no sólo el de un servicio. Sólo NB-01 cambia; las otras siete necesidades permanecen donde estaban | Analista de Negocio Senior (AG-01) |
