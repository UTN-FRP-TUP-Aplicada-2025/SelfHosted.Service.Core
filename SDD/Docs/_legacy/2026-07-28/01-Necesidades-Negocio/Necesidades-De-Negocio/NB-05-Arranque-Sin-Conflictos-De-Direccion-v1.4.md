...
| Documento | NB-05-Arranque-Sin-Conflictos-De-Direccion-v1.4.md |
| Versión | 1.4 |
| Estado | Propuesto |
| Fecha | 2026-07-28 |
| Autor | Analista de Negocio Senior (AG-01) |
| Trazabilidad upstream | SOLUTION-INTAKE §1, §3 (diferenciador 4), §4 (F-06, F-08), §5 (historia 6), §6 (flujo 3), §7 (CL-01, confirmado sin cambios el 2026-07-28 por la decisión D-4; CL-06), §17.2 P.10, §17.4 P.10, anexos E-8, E-16 (RN-03, RN-06, RN-14, RN-20), la segunda y la tercera pasada sobre la decisión D-6 del 2026-07-28, E-19; `Vision-Producto-v1.1.md` §3 (DV-04); `Alcance-Proyecto-v1.1.md` §4.1 |
...
El negocio necesita que el arranque sea previsible en las dos dimensiones. Por un lado, que el orden se deduzca de lo que está declarado y no de la memoria; y, con precisión, de aquellas dependencias en las que se declaró que hay que esperar al servicio del que se depende. Quién tiene que esperar a quién no es algo que el sistema pueda adivinar mirando la configuración: un servicio puede depender de otro y tolerar perfectamente arrancar antes, porque reintenta la conexión hasta que el otro responde, y otro puede necesitar esperar sin que nada en su configuración lo delate. Lo sabe el administrador, y por eso es él quien lo marca; el sistema le propone un valor razonable al trazar la dependencia y le deja cambiarlo. Por el otro, que el conflicto de direcciones sea una regla verificada antes de tocar el motor, con el ocupante identificado y con resoluciones concretas ofrecidas, de manera que la persona decida en lugar de investigar.
...
- El orden correcto de arranque de un conjunto de servicios no está declarado en ningún lado y se recuerda de memoria, y no hay dónde dejar asentado cuál de las dependencias entre servicios obliga a esperar y cuál no.
...
| Inmediatez del informe de conflicto | Tiempo de validación de las reservas de un proyecto SelfHosted de hasta 30 servicios, sin acceder al motor de contenedores | ≤ 50 ms | Al cierre de la etapa de direcciones y conflictos |
| Resoluciones ofrecidas | Caminos de salida concretos que acompañan a cada informe de conflicto: detener el proyecto SelfHosted que ocupa la dirección, reasignar a la siguiente libre del rango, o arrancar parcialmente | 3 de 3 resoluciones | Al cierre de la etapa de direcciones y conflictos |
| Orden de arranque deducido | Arranques de proyecto SelfHosted que respetan el orden topológico de las dependencias marcadas como de espera, sin intervención manual en el momento del arranque, sobre el total de arranques de proyecto | 100 % de los arranques | Al cierre de la etapa de arranque y parada |
...
|---|---|---|---|
| NB-05 | CU-18 arranque y parada del proyecto SelfHosted completo y de cada servicio, con marca de autoarranque | SelfHosted-Web | a generar |
...
| 1.3 | 2026-07-28 | Propagación de la tercera pasada sobre la decisión D-6, que deja sin objeto la precisión que había introducido la versión 1.2. El orden de arranque no se deduce de qué dato toma cada dependencia: esa heurística falla en los dos sentidos, porque un servicio puede tomar la dirección de otro y tolerar arrancar antes, y puede necesitar esperarlo sin tomar nada suyo. Pasa a salir de las dependencias en las que el administrador declaró que hay que esperar, con un valor propuesto por el sistema al trazarlas que no se le impone. Se reformulan la §1, el primer punto de dolor de §4 y el cuarto criterio de éxito. No cambia ningún target ni la cantidad de criterios | Analista de Negocio Senior (AG-01) |
