...
Esta carpeta contiene las ocho necesidades de negocio de la solución, generadas una sola vez a nivel solución a partir del intake único y de los documentos de contexto de la categoría 00. No se repiten por proyecto.

Las ocho están en la versión 1.1, tras la propagación del intake v1.2 del 2026-07-28. Seis cambiaron por contenido; NB-07 y NB-08 no fueron alcanzadas por ninguna de las siete decisiones y suben de versión sólo porque sus citas a `00-Contexto` debían actualizarse a la versión 1.1 de esos documentos, que cambió en paralelo.

El punto de entrada es el índice maestro [Necesidades-Negocio-v1.1.md](Necesidades-Negocio-v1.1.md), que contiene la tabla resumen, el mapa de dependencias con su verificación de aciclicidad, la trazabilidad agregada de capacidades a necesidades y de necesidades a casos de uso, y la lista consolidada de criterios de éxito derivados. Cada necesidad tiene su archivo propio en la subcarpeta `Necesidades-De-Negocio/`.
...
|---|---|---|---|---|---|
| NB-01 | Visibilidad unificada de la arquitectura de un conjunto de servicios | La arquitectura pasa a tener un registro único y legible, y habilita todo lo que se deduce de la relación entre servicios | Must Have | Propuesto | [NB-01](Necesidades-De-Negocio/NB-01-Visibilidad-Unificada-De-La-Arquitectura-v1.1.md) |
| NB-02 | Adoptabilidad del parque existente sin reinstanciar lo que ya está en producción | La solución se vuelve aplicable sobre un servidor que ya está en producción | Must Have | Propuesto | [NB-02](Necesidades-De-Negocio/NB-02-Adopcion-Del-Parque-Existente-v1.1.md) |
| NB-03 | Reproducibilidad de la arquitectura ante la pérdida del servidor | El costo de una reinstalación pasa de reconstruir a reimportar | Should Have | Propuesto | [NB-03](Necesidades-De-Negocio/NB-03-Reproducibilidad-De-La-Arquitectura-v1.1.md) |
| NB-04 | El alta de un servicio deja de ser un ejercicio de copiar y adaptar | El registro tiene quién lo alimente, y el alta deja de arrastrar decisiones ajenas | Must Have | Propuesto | [NB-04](Necesidades-De-Negocio/NB-04-Alta-De-Servicios-Sin-Copiar-Y-Adaptar-v1.1.md) |
| NB-05 | Arranque previsible: orden deducido y conflictos de dirección detectados antes de fallar | El conflicto de direcciones pasa de accidente del motor a regla verificada con resoluciones ofrecidas | Must Have | Propuesto | [NB-05](Necesidades-De-Negocio/NB-05-Arranque-Sin-Conflictos-De-Direccion-v1.1.md) |
| NB-06 | Cambios de configuración revisados antes de aplicarse y aplicados en un solo lote | Las ventanas de indisponibilidad dejan de ser proporcionales a la cantidad de cambios | Must Have | Propuesto | [NB-06](Necesidades-De-Negocio/NB-06-Cambios-Revisados-Y-Aplicados-En-Lote-v1.1.md) |
| NB-07 | Atribución del consumo del servidor a un servicio concreto | La presión sobre un servidor al límite se atribuye a un responsable concreto | Should Have | Propuesto | [NB-07](Necesidades-De-Negocio/NB-07-Atribucion-Del-Consumo-De-Recursos-v1.1.md) |
| NB-08 | Control de acceso al panel que gobierna el host y credenciales de máquina acotadas | Ninguna capacidad se expone sin credencial, y los automatismos dejan de necesitar la del administrador | Must Have | Propuesto | [NB-08](Necesidades-De-Negocio/NB-08-Control-De-Acceso-Y-Credenciales-De-Maquina-v1.1.md) |
...
| 2 | NB-01 | Establece proyecto, servicio y dependencia declarada, que es el vocabulario que usan las seis siguientes |
...
Los roles son los declarados en SOLUTION-INTAKE §2 y en `Vision-Producto-v1.1.md` §2.1. La propiedad del catálogo es del Analista de Negocio Senior; la aprobación es siempre del propietario del servidor en su rol de agente humano del proyecto.
...
El SOLUTION-INTAKE v1.2 lleva tres registros disjuntos de lo que no está cerrado, con alcances distintos, y su §19 pide expresamente no confundirlos: los supuestos `[S]`, que son valores que faltaban en las fuentes; las pendientes de decisión, que son decisiones de producto que nadie tomó; y las especificaciones de integración `[D-i]`, que el orquestador sí tomó al integrar y que están aplicadas pero son revisables. Esta sección respeta esa separación y agrega los pendientes que no vienen del intake.

Del registro de supuestos `[S]`, los seis numerados S-01 a S-06 quedaron resueltos el 2026-07-27 y queda uno sin número abierto. Del registro de pendientes de decisión, el intake declara seis. Del registro `[D-i]`, ninguna de sus entradas condiciona una necesidad de este catálogo: son formas concretas de sintaxis, códigos de respuesta y persistencia, por debajo del nivel de negocio en el que se redactan las necesidades; se declaran revisables aguas abajo y se consumen en 02-Especificacion-Funcional y en 05-Arquitectura-Tecnica.

| # | Pendiente | Registro de origen | Dueño de la resolución | Qué condiciona en este catálogo |
|---|---|---|---|---|
| 1 | Asignación de F-23 y F-24 a un alcance y a un corte vertical concreto | Pendientes de decisión, intake §4 y §15 | Agente humano del proyecto, sobre la propuesta de `07-Plan-Sprint` | El plazo del sexto criterio de éxito de NB-04, anclado a la etapa que incorpore las variables compartidas sin poder nombrarla |
| 2 | Qué hace la instanciación de una plantilla del catálogo cuando un nombre de servicio del conjunto ya existe en el proyecto destino | Pendientes de decisión, intake E-6 y RN-01 | Agente humano del proyecto | El caso de uso CU-16 de NB-04, que no puede especificar el comportamiento hasta que se cierre. Con plantillas de un solo servicio la colisión se resolvía a mano; con un conjunto de varios es mucho más probable |
| 3 | Qué hace la instanciación de una plantilla cuando declara un valor compartido cuya clave ya existe en el proyecto destino | Pendientes de decisión, intake E-6 y RN-28 | Agente humano del proyecto | El caso de uso CU-16 de NB-04. Las tres salidas posibles tienen consecuencias distintas sobre la credencial que la plantilla pretende compartir, y elegir mal expone un secreto de un conjunto de servicios a otro |
| 4 | Maquetado del paso de clasificación de variables de la incorporación y de la pantalla de valores compartidos del proyecto | Pendientes de decisión, intake E-18 | Agente humano del proyecto, sobre la propuesta de `03-UX-UI-DX` | Ninguna necesidad en su enunciado. Las dos pantallas son obligatorias por el quinto criterio de NB-02 y por el sexto de NB-04, de modo que su ausencia bloquearía la demostración de ambos |
| 5 | Distinción visual entre los dos orígenes de dependencia en el lienzo | Pendientes de decisión, intake E-18 | Agente humano del proyecto, sobre la propuesta de `03-UX-UI-DX` | Ninguna necesidad en su enunciado. NB-01 declara que la dependencia tiene dos orígenes y no distingue entre ellos; cómo se lee esa diferencia en pantalla es decisión de la categoría 03 |
| 6 | Matriz de navegadores de escritorio soportados, con familias y versiones mínimas | Figura a la vez en el registro de supuestos `[S]` sin número y en el de pendientes de decisión, intake §17.1 P.9 | Agente humano del proyecto, en su rol de usuario final | Ninguna necesidad de este catálogo. Se lista para que la enumeración de lo abierto en el intake sea completa; condiciona `03-UX-UI-DX`, `08-Calidad-Y-Pruebas` y `09-Devops` |
| 7 | Los dos criterios de éxito derivados que quedan, marcados `[D]` | Este catálogo | Agente humano del proyecto | Los criterios de aceptación correspondientes en `08-Calidad-Y-Pruebas`. Eran seis en la versión 1.0; el detalle del resultado de cada uno está en el índice maestro §5 |
| 8 | El reparto de las capacidades entre las fases 2 y 3 | `Roadmap-Producto-v1.1.md` §2.3 | Agente humano del proyecto, sobre la propuesta del roadmap | Los plazos de los criterios de éxito de NB-03, NB-04 y NB-07, expresados por cierre de fase |

Tres pendientes de la versión anterior se cerraron el 2026-07-28 y por eso ya no figuran: el caso límite CL-04, por la decisión D-1; el caso límite CL-15, por la decisión D-2; y el supuesto IC-05, por la decisión D-3. Las advertencias que NB-02 llevaba por los dos últimos se retiraron en su versión 1.1.
