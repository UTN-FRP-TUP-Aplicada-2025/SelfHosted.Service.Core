...
| Documento | NB-02-Adopcion-Del-Parque-Existente-v1.0.md |
| Versión | 1.0 |
| Estado | Propuesto |
| Fecha | 2026-07-27 |
| Autor | Analista de Negocio Senior (AG-01) |
| Trazabilidad upstream | SOLUTION-INTAKE §1 (disparador), §3 (diferenciador 1), §4 (F-11), §5 (historia 7), §6 (flujo 2), §7 (CL-03, CL-07, CL-08, CL-15), §8, §11 (RG-09), §17.4 P.11 (supuesto IC-05, invariante I10), anexos E-7, E-11, E-19; `Vision-Producto-v1.0.md` §3 (DV-01), §5 (OBJ-01); `Alcance-Proyecto-v1.0.md` §4.1, §6.2 |
...
Advertencia sobre el tercer punto de dolor. El supuesto IC-05 del intake sigue abierto y está declarado como pendiente del Sprint 0: la interpretación de que un contenedor no puede quedar incorporado a dos proyectos a la vez proviene de una frase cortada de la definición del servicio, y el análisis la formalizó en la invariante I10. Las reglas de incorporación derivadas de esa invariante ya están declaradas de forma cerrada, de modo que el punto de dolor y el caso de uso CU-06 se redactan sobre ella. Si la confirmación del Sprint 0 desmintiera esa lectura, lo que cambia es el fundamento de este punto y el tratamiento del contenedor ya incorporado en CU-06, no los criterios de éxito de §5: ninguno de los cinco mide esa regla. La resolución es del agente humano del proyecto.

## 5. Criterios de éxito

| Criterio | Métrica | Target | Plazo |
|---|---|---|---|
| Adopción del parque existente | Contenedores en ejecución del servidor de referencia incorporados a un proyecto de la solución sin haber sido reinstanciados, sobre los 8 del parque relevado | ≥ 6 de 8 contenedores (≥ 75 %) | 3 meses desde el cierre de la Fase 1 |
...
| Contención de credenciales importadas | Variables de entorno que la heurística de sensibilidad identifica y que se importan enmascaradas, sobre el total de variables identificadas | 100 % enmascaradas | Continuo |

Filas derivadas. Ninguna fila de esta tabla está derivada. La primera es la métrica de adopción del parque de SOLUTION-INTAKE §8; el resto toma su número del dimensionamiento verificado del parque, de la enumeración de dimensiones del flujo de adopción, de las cinco salvaguardas de aislamiento declaradas en el alcance y de la regla de enmascarado por heurística.

Advertencia sobre el último criterio. El caso límite CL-15 sigue abierto: una variable que contiene un secreto pero cuyo nombre no coincide con la heurística se importaría en claro. El criterio mide lo que la heurística identifica, no la totalidad de los secretos existentes, y su alcance definitivo depende de la resolución que el cliente adopte.
...
| NB-02 | CU-07 incorporación de un contenedor existente a un proyecto, con confirmación explícita | SelfHosted-Web | a generar |
...
- Depende de: NB-01, porque el contenedor incorporado se asocia a un proyecto y se representa como servicio del registro; y NB-05, porque la dirección observada del contenedor debe reservarse sin entrar en conflicto con un servicio activo de otro proyecto.
