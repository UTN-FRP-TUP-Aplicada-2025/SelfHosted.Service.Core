...
**Trazabilidad upstream:** SOLUTION-INTAKE v1.1 §1, §4, §5, §6, §7, §8, §9, §10, §11, §15, más su tabla de estado de supuestos
...
El alcance se expresa en lenguaje de negocio. Las decisiones de plataforma, versiones y matrices de soporte están en `Compatibilidad-Plataformas-v1.0.md`; la secuencia de construcción está en `Roadmap-Producto-v1.0.md`; las reglas de trabajo del equipo están en `Acuerdo-Equipo-v1.0.md`.
...
|---|---|---|
| OP-01 | Entregar un registro único y navegable de la arquitectura de cada conjunto de servicios del servidor | OBJ-01 y OBJ-02 de `Vision-Producto-v1.0.md` |
...
Las capacidades se enumeran con el identificador del SOLUTION-INTAKE §4, que es el que consumen las categorías 01 y 02. La prioridad MoSCoW traduce la pertenencia de cada capacidad a uno de los cuatro alcances incrementales; el reparto por fases está en `Roadmap-Producto-v1.0.md`.
...
| F-13 | Exportación e importación de la arquitectura completa de un proyecto en formato estándar de composición, más el manifiesto propio que preserva la disposición del lienzo | Should Have |
...
|---|---|---|---|
| Desarrollo | Entorno contenedorizado declarativo sobre el host de desarrollo, con acceso al motor de contenedores del propio host | Equipo de desarrollo y agente IA de codificación | El resultado se observa desde el navegador del host. El detalle de plataforma está en `Compatibilidad-Plataformas-v1.0.md` |
...
Los seis valores que el SOLUTION-INTAKE registró como supuestos numerados S-01 a S-06 están cerrados. No son los únicos datos que el intake marcó como supuesto: la matriz de navegadores de §17.1 P.9 y el supuesto IC-05 de §17.4 P.11 llevan el mismo marcador sin número, no están en la tabla de estado y siguen abiertos, de modo que se registran en §6.2 y no acá. El orquestador los presentó en su batería de validación y el agente humano del proyecto los resolvió el 2026-07-27; la resolución de cada uno está publicada en la tabla de estado de supuestos del SOLUTION-INTAKE v1.1, que es la fuente de verdad de ese estado y el respaldo de esta sección.
...
|---|---|---|---|---|
| S-01 | Las métricas de éxito de negocio y sus umbrales | Confirmado el 2026-07-27 | Valores propuestos por el intake, adoptados sin cambios: son el objetivo de negocio de la solución | `Vision-Producto-v1.0.md` §6 y los objetivos OBJ-01 a OBJ-04 |
| S-02 | Los umbrales mínimos de cobertura de pruebas por proyecto | Confirmado el 2026-07-27 | Los cuatro pares de umbrales se adoptan sin cambios y son control bloqueante del pipeline | `Acuerdo-Equipo-v1.0.md` §5 y la categoría 08-Calidad-Y-Pruebas |
...
| S-04 | El esquema de versionado y la convención de mensajes de cambio, sobre el etiquetado por etapa que las fuentes sí declaran | Confirmado el 2026-07-27 | Esquema propuesto adoptado sin cambios | `Acuerdo-Equipo-v1.0.md` §4.3 |
| S-05 | La ubicación del repositorio remoto | Resuelto con evidencia el 2026-07-27 | Deja de ser supuesto: el remoto existe y está configurado, y el dato es verificable en el entorno con `git remote get-url origin` en la raíz del repositorio destino | `Acuerdo-Equipo-v1.0.md` §7 y la categoría 09-Devops |
| S-06 | Los nombres propios del propietario del problema y del agente humano del proyecto | Cerrado por identificación de rol el 2026-07-27 | No se aportan nombres propios y no se requieren: los actores se identifican por su rol, unívoco porque el propietario, el lead técnico y el usuario final son la misma persona. Ningún artefacto downstream debe pedir un nombre propio | `Vision-Producto-v1.0.md` §2 y el README de esta sección |

### 6.2 Supuestos abiertos

Cuatro pendientes del SOLUTION-INTAKE siguen sin resolver: dos casos límite de §7, el supuesto IC-05 de §17.4 P.11 y la matriz de navegadores de §17.1 P.9. La batería de validación del 2026-07-27, que cerró los seis supuestos numerados de §6.1, no alcanzó a ninguno de los cuatro: el SOLUTION-INTAKE v1.1 los enumera como pendientes en la tabla de su §19 y en la tabla de marcadores sin número de su sección de supuestos, que declaran el mismo conjunto, y esas tablas son el respaldo de esta sección.

Este documento no les propone respuesta. Cada pendiente tiene su propio consumidor y no todos caen en la misma categoría: CL-04 y CL-15 condicionan a 02-Especificacion-Funcional, que no puede especificar el comportamiento correspondiente hasta que se cierren; IC-05 condiciona el fundamento de una invariante del modelo en 02-Especificacion-Funcional de SelfHosted-Domain; y la matriz de navegadores condiciona a 03-UX-UI-DX y la verificación de compatibilidad en 08-Calidad-Y-Pruebas y 09-Devops.

| ID | Supuesto abierto | Qué queda sin definir | Quién resuelve | Qué bloquea |
|---|---|---|---|---|
| CL-04 | Comportamiento ante la caída de la conexión del navegador con un despliegue en curso | Qué debe ocurrir con la operación iniciada, con el estado que ve el usuario al reconectar y con la notificación del resultado | Cliente | El caso de uso de despliegue y sus flujos alternativos en 02-Especificacion-Funcional; el comportamiento de reconexión en 03-UX-UI-DX |
| CL-15 | Tratamiento de un secreto contenido en una variable cuyo nombre no coincide con la heurística de sensibilidad | Cuál de las resoluciones posibles se adopta para que la adopción de un contenedor no importe una credencial en claro | Cliente | La regla de negocio de enmascarado en la adopción, en 02-Especificacion-Funcional; la mitigación del riesgo RG-09 en 05-Arquitectura-Tecnica |
| IC-05 | Confirmación de que la verificación pendiente es que el contenedor no esté ya adoptado por otro proyecto, formalizada en la invariante I10 | Si esa lectura del enunciado original es la correcta, que es lo que el intake declara abierto para el Sprint 0 | Agente humano del proyecto, que es quien cierra el Sprint 0 | No acota ningún criterio de aceptación de este alcance: sostiene el fundamento de la invariante I10 en 02-Especificacion-Funcional de SelfHosted-Domain y el tratamiento del contenedor ya incorporado en el caso de uso de adopción de SelfHosted-Web. Se registra acá por completitud del inventario de pendientes, no porque sea una decisión de contexto de solución |
| Matriz de navegadores | Qué familias y versiones mínimas de navegador de escritorio se soportan | El intake declara la exigencia de conexión persistente por WebSockets, pero no la familia ni la versión mínima | Cliente | El alcance de lo que 03-UX-UI-DX puede prometer sobre el comportamiento de la interfaz, y la verificación de compatibilidad en 08-Calidad-Y-Pruebas y 09-Devops. Queda declarada como brecha en `Compatibilidad-Plataformas-v1.0.md` §2.2 |
...
Las restricciones del cliente están enumeradas en `Vision-Producto-v1.0.md` §7 como RE-01 a RE-12. Este documento agrega la lectura de cada una en clave de alcance.
...
| RE-10 | Una rama y un pull request por etapa, con etiqueta por etapa cerrada y ningún secreto en el repositorio | Sin efecto sobre las capacidades del producto, pero sí sobre los entregables: condiciona EN-03 y EN-07, y su detalle operativo vive en `Acuerdo-Equipo-v1.0.md` §4 |
...
| CA-04 | Las reglas de negocio que introduce cada etapa están cubiertas por pruebas automatizadas | Verificación de cobertura en el control de calidad del pipeline, con los umbrales por proyecto declarados en `Acuerdo-Equipo-v1.0.md` §5 |
...
| CA-10 | Los dos supuestos abiertos CL-04 y CL-15 están resueltos por el cliente antes de que se especifique la etapa que los necesita | Registro de la respuesta en el intake y propagación a 02-Especificacion-Funcional |
...
| 5. Propagación | Un cambio aceptado obliga a revisar `Roadmap-Producto-v1.0.md` y las categorías 01, 02 y 06. El cambio no se considera cerrado hasta que esa propagación está hecha |
...
| 3. Objetivos del proyecto | §8, a través de los objetivos SMART de `Vision-Producto-v1.0.md` |
...
| 6. Supuestos | Tabla de estado de supuestos del intake v1.1 para §6.1; §7 para los casos límite CL-04 y CL-15, §17.1 P.9 para la matriz de navegadores y la tabla de pendientes de §19 para los tres, que siguen abiertos, en §6.2 |
| 7. Restricciones | §10 |
...
|---|---|
| 01-Necesidades-Negocio | Las capacidades F-01 a F-17 como origen de cada necesidad de negocio, y los objetivos OP-01 a OP-07 |
...
|---|---|---|---|
| 1.0 | 2026-07-27 | Versión inicial, derivada del SOLUTION-INTAKE-SelfHosted-Service-Core-v1.1. Diecisiete capacidades incluidas, trece exclusiones justificadas, seis supuestos del intake tratados entonces como confirmados —hoy diferenciados en cuatro confirmados, uno resuelto con evidencia y uno cerrado por identificación de rol, según la tercera fila—, dos supuestos abiertos y diez criterios de aceptación | Product Manager Senior (AG-00) con Analista de Negocio Senior (AG-01) |
