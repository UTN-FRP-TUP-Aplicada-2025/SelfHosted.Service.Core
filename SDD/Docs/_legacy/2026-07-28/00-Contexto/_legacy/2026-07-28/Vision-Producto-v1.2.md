...
**Documento:** Vision-Producto-v1.2.md
**Versión:** 1.2
**Estado:** Propuesto
**Fecha:** 2026-07-28
...
El avance hacia ese estado no se mide por fechas de calendario. El plazo declarado por el cliente es explícitamente "sin fecha objetivo": el progreso se mide por etapas cerradas, cada una con su punto de control aprobado. La secuencia de alcances y las condiciones de transición están en `Roadmap-Producto-v1.2.md`.
...
El detalle de cada exclusión, con su justificación y su versión futura tentativa, está en `Alcance-Proyecto-v1.2.md` §5.
...
| RE-10 | Flujo de trabajo obligatorio | Una rama y un pull request por etapa, siendo el pull request el punto de control; etiqueta por etapa cerrada; ningún secreto entra al repositorio | Las reglas operativas están en `Acuerdo-Equipo-v1.2.md` |
...
| Arista o enlace | Conexión dibujada en el lienzo. Representa que un servicio consume, por variable de entorno, un dato de otro servicio del mismo proyecto. Toda arista nace de una referencia: trazar la flecha en el lienzo es la forma cómoda de escribirla, no un mecanismo aparte | Enlace. Según qué se referencie, la arista es de red —apunta a la dirección o al puerto del destino, y por lo tanto ordena el arranque y exige que el destino esté alcanzable— o de dato —apunta a cualquier otra variable, y no condiciona el arranque, porque el valor se resuelve del registro |
...
| Referencia de variable | Valor de una variable expresado como un apuntador a otra variable en lugar de como literal: a otra del propio servicio, a una compartida del proyecto o a una de otro servicio del mismo proyecto | Es el único mecanismo de vínculo entre servicios: lo que traza el lienzo es una referencia. El contenedor nunca ve el apuntador, ve el valor ya resuelto. La sintaxis concreta es materia de 02-Especificacion-Funcional |
...
| Variable provista por el sistema | Variable de sólo lectura que el sistema expone en cada servicio sin que nadie la declare: su dirección interna, su puerto y su nombre, más el del proyecto | Es lo que permite que un solo mecanismo alcance: un servicio toma la dirección de otro referenciándola como referenciaría cualquier variable. Ninguna es secreta, y sus nombres concretos son materia de 02-Especificacion-Funcional |
