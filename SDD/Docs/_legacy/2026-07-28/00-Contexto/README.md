...
**Solución:** SelfHosted.Service.Core (proyecto de código principal `SelfHosted-Web`)
**Documento:** README.md
**Estado:** Vigente
**Fecha:** 2026-07-28
**Autor:** Product Manager Senior (AG-00) en conjunción con Analista de Negocio Senior (AG-01)
**Trazabilidad upstream:** SOLUTION-INTAKE v1.2 §2 para los stakeholders de §4, §7 y su tabla de estado de supuestos para las preguntas abiertas de §5, §13 para la identidad de la solución y su composición; SOLUTION-MANIFEST §1 y §2 para el proyecto de código principal y su tipo, que determinan la variante de especialidad de la categoría
...
Esta carpeta es el contexto de producto de la solución completa. Se genera una sola vez a nivel solución, no por proyecto de código, y es el inicio de la cadena de trazabilidad: no tiene documentos aguas arriba dentro de la especificación, sólo el intake de la solución y el manifiesto derivado de él.
...
|---|---|---|---|
| 1 | [Vision-Producto](Vision-Producto-v1.5.md) | Por qué existe el sistema: problema de negocio, audiencia, propuesta de valor, visión a tres años, objetivos SMART, métricas de éxito, restricciones, riesgos y glosario del dominio | Propuesto |
| 2 | [Alcance-Proyecto](Alcance-Proyecto-v1.5.md) | Qué entra y qué no entra: capacidades, entregables, ambientes, exclusiones justificadas, supuestos, criterios de aceptación y gestión de cambios de alcance | Propuesto |
| 3 | [Roadmap-Producto](Roadmap-Producto-v1.5.md) | En qué secuencia se construye: fases, épicas, etapas, dependencias, puertas técnicas y criterios verificables de transición | Propuesto |
| 4 | [Compatibilidad-Plataformas](Compatibilidad-Plataformas-v1.4.md) | Sobre qué plataformas corre: matriz por componente, versiones mínimas con motivo, restricciones justificadas, alternativas y estado de verificación previsto | Propuesto |
| 5 | [Acuerdo-Equipo](Acuerdo-Equipo-v1.4.md) | Con qué reglas trabaja el equipo: roles, ceremonias disparadas por evento, ramas, revisión, convenciones, documentación, secretos, definición de terminado y de listo, y herramientas | Propuesto |
...
Ninguna bloquea la generación de esta categoría, pero las cuatro condicionan categorías posteriores y su respuesta es del cliente. El registro se actualizó por última vez el 2026-07-28, cuando las dos preguntas de instanciación del catálogo se cerraron: la de la clave de variable compartida desapareció por modelo, porque las relaciones pasaron a establecerse por identidad y la colisión dejó de ser posible, y la del nombre de servicio se resolvió con sufijo automático más aviso.

| # | Pregunta | Dónde queda registrada | Qué condiciona |
|---|---|---|---|
| 1 | Qué familias y versiones de navegador se soportan | `Compatibilidad-Plataformas` §2.2 y `Alcance-Proyecto` §6.2 | La verificación de compatibilidad de interfaz en 08-Calidad-Y-Pruebas y 09-Devops, y el alcance de lo que 03-UX-UI-DX puede prometer |
| 2 | Qué capacidades componen exactamente los alcances 2 y 3, más allá de los dos anclajes que el intake fija | `Roadmap-Producto` §2.3 | El armado de épicas de 06-Backlog y la secuencia de 07-Plan-Sprint |
| 3 | Si se adopta como objetivo de negocio la formulación de OBJ-05, que traduce a meta de producto los umbrales de la puerta técnica PT-01 y el dimensionamiento del intake | `Vision-Producto` §5, nota de origen de los datos | La lectura y el responsable de esa métrica en 08-Calidad-Y-Pruebas. Es una derivación propia de esta categoría, no un dato declarado por el intake |
| 4 | En qué fase y en qué corte vertical entran las capacidades nuevas F-23, F-24 y F-25 | `Roadmap-Producto` §2.4 | La secuencia de etapas de 07-Plan-Sprint. Los diez cortes del primer alcance están cerrados y ninguna de las tres figura entre ellos |
...
- La disponibilidad y los tiempos de respuesta, que el SOLUTION-INTAKE §10 registra como restricción del cliente —sin horario core, sin franja comprometida y sin plazo máximo, con el bloqueo del punto de control que no vence—, traducida a regla operativa en `Acuerdo-Equipo` §4.6.
...
- Las dos decisiones sobre la instanciación de un ítem del catálogo, cerradas el 2026-07-28: la de la clave de variable compartida desapareció por modelo y la del nombre de servicio se resolvió con sufijo automático más aviso. Están detalladas en `Alcance-Proyecto` §6.2.
...
| 1.2 | 2026-07-28 | Propagación de la segunda pasada sobre el mecanismo de variables del SOLUTION-INTAKE v1.2. §2 actualiza los cinco documentos a la versión 1.2 y §5 sus referencias. El registro de preguntas abiertas no cambia de contenido: las seis siguen abiertas y ninguna de las decisiones de esta pasada las alcanza. Los cinco documentos versionados suben a 1.2 y sus versiones 1.1 quedan archivadas en `_legacy/2026-07-28/`, junto a las 1.0 | Product Manager Senior (AG-00) con Analista de Negocio Senior (AG-01) |
| 1.3 | 2026-07-28 | Propagación de la tercera pasada sobre el mecanismo de variables del SOLUTION-INTAKE. §2 actualiza los cinco documentos a la versión 1.3 y §5 sus referencias. El registro de preguntas abiertas no cambia: las seis siguen abiertas y ninguna decisión de esta pasada las alcanza. Los cinco documentos versionados suben a 1.3 y sus versiones 1.2 quedan archivadas en `_legacy/2026-07-28/`, el README con sufijo de versión al archivarse, según la convención de la solución | Product Manager Senior (AG-00) con Analista de Negocio Senior (AG-01) |
| 1.4 | 2026-07-28 | Cuarta pasada del SOLUTION-INTAKE, de terminología. Se califican como proyecto de código las cinco menciones en ese sentido: el nivel de generación de la categoría en §1, el proyecto de código principal de la trazabilidad upstream y de la cabecera, y las tres de la declaración de completitud de §3. §2 pasa a citar los cinco documentos por su nombre sin versión, con el enlace apuntando al archivo real. Se adopta además la convención de citas de la solución: las citas en prosa van sin sufijo de versión y el nombre versionado queda reservado a la identidad del propio documento, a la trazabilidad upstream de la cabecera y a los enlaces navegables, que necesitan el nombre de archivo real. Es lo que corta la cascada por la que un documento subía de versión sin tener una línea propia que corregir. Los cinco documentos versionados suben a 1.4 y sus versiones 1.3 quedan archivadas en `_legacy/2026-07-28/`, el README con sufijo de versión al archivarse | Product Manager Senior (AG-00) con Analista de Negocio Senior (AG-01) |
| 1.5 | 2026-07-28 | Quinta y última pasada del SOLUTION-INTAKE. §5 baja de seis preguntas abiertas a cuatro: las dos de instanciación del catálogo se cerraron y pasan a la lista de lo resuelto, con la distinción de que una desapareció por modelo y la otra se resolvió con una regla; la pregunta por la ubicación en una fase incorpora F-25. §2 actualiza a 1.5 los tres documentos que cambiaron. `Compatibilidad-Plataformas` y `Acuerdo-Equipo` quedan en 1.4: la convención de citas sin versión evitó que subieran sin motivo propio, que es lo que venía pasando en las tres rondas anteriores | Product Manager Senior (AG-00) con Analista de Negocio Senior (AG-01) |
