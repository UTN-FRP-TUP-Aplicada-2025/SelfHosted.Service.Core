...
**Documento:** Vision-Producto-v1.5.md
**Versión:** 1.5
**Estado:** Propuesto
**Fecha:** 2026-07-28
**Autor:** Product Manager Senior (AG-00) en conjunción con Analista de Negocio Senior (AG-01)
**Trazabilidad upstream:** SOLUTION-INTAKE v1.2 §1, §2, §3, §4, §5, §8, §9, §10, §11, §12, más su tabla de estado de supuestos
...
El alta de un servicio nuevo deja de ser un ejercicio de copiar y adaptar. El catálogo del propietario, que arranca vacío y se puebla con lo que él mismo guarda, le permite volver a levantar de una sola vez un conjunto de servicios que ya resolvió antes, con los valores que cambian pedidos como parámetros. Y un valor que varios servicios comparten —típicamente una credencial— se define una sola vez en el proyecto y se referencia desde donde haga falta, en lugar de escribirse y mantenerse sincronizado servicio por servicio.

La publicación de una versión nueva puede dispararse desde un automatismo con una credencial de ámbito mínimo, sin que ningún flujo de trabajo conozca la contraseña del administrador.

El avance hacia ese estado no se mide por fechas de calendario. El plazo declarado por el cliente es explícitamente "sin fecha objetivo": el progreso se mide por etapas cerradas, cada una con su punto de control aprobado. La secuencia de alcances y las condiciones de transición están en `Roadmap-Producto`.
...
El detalle de cada exclusión, con su justificación y su versión futura tentativa, está en `Alcance-Proyecto` §5.
...
| RE-10 | Flujo de trabajo obligatorio | Una rama y un pull request por etapa, siendo el pull request el punto de control; etiqueta por etapa cerrada; ningún secreto entra al repositorio | Las reglas operativas están en `Acuerdo-Equipo` |
...
| RG-09 | Secretos importados durante la adopción que terminen visibles | Media | Alto | Mitigación reforzada por la decisión D-2 del agente humano del proyecto del 2026-07-28: el carácter de secreto se declara y no se infiere del nombre de la variable. La adopción de un contenedor no se completa sin un paso obligatorio en el que el administrador clasifica cada variable importada; la heurística por nombre deja de decidir y pasa a sugerir. Se mantiene la regla de no devolver secretos en texto plano ni por la interfaz de máquina ni en exportaciones. Con esa decisión queda cerrado el caso límite CL-15, que hasta entonces era la brecha de esta mitigación | Equipo de desarrollo |
...
|---|---|---|
| Adopción | Incorporación de un contenedor ya existente en el servidor a un proyecto SelfHosted, sin recrearlo: sólo se importa su configuración y se lo vincula por identificador | Es el diferenciador DV-01 |
...
| Arista o enlace | Conexión dibujada en el lienzo. Representa que un servicio depende de otro del mismo proyecto. Casi siempre nace de una referencia —trazar la flecha es la forma cómoda de escribirla—, y también puede existir sin variable de por medio, cuando su única razón es esperar al destino | Enlace. Tiene dos ejes independientes. Uno es la espera al destino, que el administrador declara y que define el orden de arranque; el sistema la propone al trazar la flecha, pero la decisión es del usuario. El otro es si la arista toma la dirección del destino, lo que exige que haya canal alcanzable entre ambos |
...
| Catálogo | Colección de plantillas reutilizables, editable, exportable e importable. Es la cuarta vía de alta de un servicio, junto a las tres variantes de origen | Nada del catálogo corre: sus ítems son definiciones en reposo. Arranca vacío en una instalación nueva y se puebla con lo que el propietario guarda o importa |
...
| Higiene del modelo | Conjunto de condiciones que el sistema detecta y advierte sin bloquear: una variable compartida que nadie referencia, dos elementos con el mismo nombre visible, una clave que ya existe al instanciar una plantilla y una referencia que quedó sin uso | Informa, nunca impide. Es lo que entrega la capacidad F-25 |
...
| Informe de importación | Resultado que devuelve la importación de la arquitectura de un proyecto desde un archivo de composición: qué se creó y, sobre todo, qué no se pudo representar | Es lo que hace que una pérdida de traducción sea declarada y no silenciosa. Sin él, el administrador creería haber importado algo que quedó incompleto. Desde el 2026-07-28 el orden de arranque declarado en el archivo de origen dejó de ser una de esas pérdidas: se importa como arista con espera |
...
| Objeto con identidad | Elemento del modelo que tiene identidad propia, de modo que lo que lo referencia lo hace por esa identidad y nunca por su nombre. El nombre es un atributo suyo, y su unicidad se exige sólo donde cumple una función real | Es lo que hace que renombrar sea seguro y que dos elementos puedan llamarse igual sin ambigüedad. Qué elementos del modelo lo son es materia de 02-Especificacion-Funcional y 05-Arquitectura-Tecnica |
| Política de reinicio | Regla que indica si el contenedor debe reiniciarse solo, y bajo qué condición | — |
| Proyecto SelfHosted | Lo que el administrador crea desde el portal: la arquitectura completa de servicios contenedorizados, con su red y su lienzo. Es lo que designa «proyecto» a secas cuando el contexto ya lo fijó | Se escribe `SelfHosted` sin guion y sin sufijo, precedido de «proyecto» |
| Proyecto de código | Cada una de las cuatro unidades de la composición de la solución: `SelfHosted-Web`, `SelfHosted-Application`, `SelfHosted-Infrastructure` y `SelfHosted-Domain`. Es una unidad de compilación, no algo que el administrador vea | Se escribe siempre completo, nunca «proyecto» a secas. El nombre de código lleva guion y sufijo de rol y no va precedido de «proyecto»: la construcción «el proyecto SelfHosted-Web» está prohibida porque fusiona los dos términos |
...
| Referencia de variable | Valor de una variable expresado como un apuntador a otra variable en lugar de como literal: a otra del propio servicio, a una compartida del proyecto o a una de otro servicio del mismo proyecto | Queda vinculada al elemento que apunta y no a su nombre, de modo que renombrarlo no la rompe: renombrar cualquier cosa es una operación de presentación. El contenedor nunca ve el apuntador, ve el valor ya resuelto. La sintaxis concreta es materia de 02-Especificacion-Funcional |
...
| Subgrafo parametrizado | Contenido de un ítem del catálogo: uno o varios servicios con sus aristas y con huecos parametrizables. Al instanciarlo se crean tantos servicios, y tantos contenedores, como nodos tenga | Es lo que distingue a un ítem del catálogo de una plantilla de un solo servicio |
...
| Variable compartida del proyecto | Variable definida una sola vez a nivel proyecto y referenciable desde cualquiera de sus servicios. Puede ser secreta, con el mismo tratamiento que cualquier otro secreto | Resuelve el valor que hoy hay que repetir y mantener sincronizado servicio por servicio. Dos variables compartidas pueden llamarse igual sin ambigüedad, porque lo que las distingue es su identidad y no su nombre |
| Variable de enlace | Variable de entorno que el sistema escribe al trazar una arista en el lienzo | No es una clase aparte de variable: es una referencia común a las variables que el destino provee, y el administrador puede editarla como cualquier otra |
| Variable provista por el sistema | Variable de sólo lectura que el sistema expone en cada servicio sin que nadie la declare. Son dos: su dirección interna y su nombre | Es lo que permite que un servicio tome la dirección de otro referenciándola como referenciaría cualquier variable. El puerto no es una de ellas: lo declara el administrador y se escribe como valor. Ninguna es secreta, y sus nombres concretos son materia de 02-Especificacion-Funcional |
...
| 1.1 | 2026-07-28 | Propagación de las siete decisiones del agente humano del proyecto del 2026-07-28, consolidadas en el SOLUTION-INTAKE v1.2. §4.1 reescribe el párrafo del catálogo, que dejaba leerlo como un conjunto de servicios, y suma la definición de un valor una sola vez por proyecto (D-5, D-7). §8 reescribe la mitigación de RG-09: el carácter de secreto se declara en un paso obligatorio de clasificación y la heurística pasa a sugerir, con lo que queda cerrado CL-15 (D-2). §9 redefine «Arista o enlace» por sus dos orígenes y suma cuatro términos —Catálogo, Referencia de variable, Subgrafo parametrizado y Variable compartida del proyecto—, de 29 a 33 (D-5, D-6, D-7). Sube a 1.1 porque el documento ya fue consumido por 01-Necesidades-Negocio; la 1.0 queda archivada en `_legacy/2026-07-28/`. Se actualizaron las referencias al intake y las referencias cruzadas a los documentos de la categoría. Las citas actualizadas son las vivas —cabecera, cuerpo y enlaces navegables—; las filas anteriores de este control de cambios conservan la versión del intake y los nombres de archivo contra los que se trabajó cada día, porque son parte del hecho registrado. | Product Manager Senior (AG-00) con Analista de Negocio Senior (AG-01) |
| 1.2 | 2026-07-28 | Propagación de la segunda pasada sobre el mecanismo de variables del SOLUTION-INTAKE v1.2, que unificó el vínculo entre servicios en lugar de mantener dos. §9 es el cambio de fondo: «Arista o enlace» se redefine —toda arista nace de una referencia, y se distingue la de red, que ordena el arranque, de la de dato, que no—; «Variable de enlace» deja de ser una clase aparte y pasa a ser una referencia común y editable; y entran «Informe de importación» y «Variable provista por el sistema», con lo que el glosario pasa de 33 a 35 términos. «Referencia de variable» declara ahora que es el único mecanismo de vínculo. No se transcriben ni la sintaxis ni los nombres concretos de las variables del sistema, que son de 02 y 05 | Product Manager Senior (AG-00) con Analista de Negocio Senior (AG-01) |
| 1.3 | 2026-07-28 | Propagación de la tercera pasada sobre el mecanismo de variables del SOLUTION-INTAKE, que volvió a simplificar el modelo tras contrastarlo contra la plataforma de referencia. §9: «Arista o enlace» se redefine sobre dos ejes independientes —la espera al destino, que el administrador declara y que define el orden de arranque, y la toma de la dirección del destino, que exige canal alcanzable— y admite la arista sin variable de por medio; «Referencia de variable» declara que queda vinculada al servicio y no a su nombre, con lo que renombrar deja de romperla; «Variable provista por el sistema» se reduce a dos claves y declara que el puerto no es una de ellas; «Informe de importación» declara que el orden de arranque del archivo de origen dejó de ser una pérdida de traducción. El glosario se mantiene en 35 términos. No se transcriben ni la sintaxis, ni los nombres de las claves provistas, ni el mecanismo del vínculo | Product Manager Senior (AG-00) con Analista de Negocio Senior (AG-01) |
| 1.4 | 2026-07-28 | Cuarta pasada del SOLUTION-INTAKE, de terminología: ninguna regla, flujo ni decisión cambió. §9 parte la entrada «Proyecto», que advertía la ambigüedad sin resolverla, en «Proyecto SelfHosted» y «Proyecto de código», cada una con su regla léxica y con la prohibición de fusionarlas; el glosario pasa de 35 a 36 términos. La definición de «Adopción», que es la primera del glosario que menciona el término, usa la forma completa. La cabecera califica el proyecto de código principal. Se adopta además la convención de citas de la solución: las citas en prosa van sin sufijo de versión y el nombre versionado queda reservado a la identidad del propio documento, a la trazabilidad upstream de la cabecera y a los enlaces navegables, que necesitan el nombre de archivo real. Es lo que corta la cascada por la que un documento subía de versión sin tener una línea propia que corregir. | Product Manager Senior (AG-00) con Analista de Negocio Senior (AG-01) |
| 1.5 | 2026-07-28 | Quinta y última pasada del SOLUTION-INTAKE. §9 suma «Higiene del modelo» y «Objeto con identidad», con lo que el glosario pasa de 36 a 38 términos; la segunda se define por lo que habilita y no como decisión de modelado, que es materia de 02 y 05. Se extienden dos entradas con las promesas que el principio de identidad hace posibles: «Referencia de variable» pasa de decir que renombrar un servicio no la rompe a decir que no la rompe renombrar ningún elemento, y «Variable compartida del proyecto» declara que dos pueden llamarse igual sin ambigüedad | Product Manager Senior (AG-00) con Analista de Negocio Senior (AG-01) |
