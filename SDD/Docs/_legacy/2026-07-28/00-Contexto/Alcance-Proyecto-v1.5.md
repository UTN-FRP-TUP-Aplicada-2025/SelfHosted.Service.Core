...
**Documento:** Alcance-Proyecto-v1.5.md
**Versión:** 1.5
**Estado:** Propuesto
**Fecha:** 2026-07-28
...
El alcance se expresa en lenguaje de negocio. Las decisiones de plataforma, versiones y matrices de soporte están en `Compatibilidad-Plataformas`; la secuencia de construcción está en `Roadmap-Producto`; las reglas de trabajo del equipo están en `Acuerdo-Equipo`.

## 2. Descripción general

El proyecto construye un administrador de servicios contenedorizados para un único servidor autoalojado y un único administrador. Su unidad de trabajo es el proyecto SelfHosted: la arquitectura completa de servicios que el administrador crea desde el portal, con su modo de red, sus direcciones reservadas y su disposición en un lienzo visual editable. En este documento «proyecto» a secas designa siempre ese sentido; la unidad de compilación se escribe completa, «proyecto de código».
...
|---|---|---|
| OP-01 | Entregar un registro único y navegable de la arquitectura de cada conjunto de servicios del servidor | OBJ-01 y OBJ-02 de `Vision-Producto` |
...
Las capacidades se enumeran con el identificador del SOLUTION-INTAKE §4, que es el que consumen las categorías 01 y 02. La prioridad MoSCoW traduce la pertenencia de cada capacidad a uno de los cuatro alcances incrementales; el reparto por fases está en `Roadmap-Producto`.
...
| F-06 | Arranque y parada del proyecto completo y de cada servicio, con marca de autoarranque y respeto del orden de arranque que declara el grafo de dependencias | Must Have |
...
| F-25 | Higiene del modelo: el sistema detecta y advierte, sin bloquear, una variable compartida que nadie referencia, dos elementos con el mismo nombre visible, una clave que ya existe al instanciar una plantilla y una referencia que quedó sin uso | Could Have |

Tres precisiones de alcance incorporadas el 2026-07-28 por decisión del agente humano del proyecto, según la sección de decisiones del SOLUTION-INTAKE v1.2. Primera, sobre F-14: el catálogo es una vía de alta y no un origen de imagen más, nada de lo que contiene está corriendo, arranca vacío en una instalación nueva y un ítem puede traer varios servicios con sus dependencias, no uno solo. Segunda, F-23 y F-24 son capacidades nuevas que resuelven el mismo dolor —un valor que hoy hay que escribir y mantener sincronizado en cada servicio que lo usa— y entran como Should Have porque los diez cortes verticales del primer alcance están declarados de forma cerrada y ninguna de las dos figura entre ellos. Tercera, su ubicación en una fase concreta es una pendiente declarada, registrada en §6.2 y en `Roadmap-Producto` §2.4.

Una precisión posterior, del 2026-07-28, sobre cómo se relacionan F-04, F-06 y F-24. El vínculo entre dos servicios es un mecanismo único: trazar la arista en el lienzo escribe una referencia, de modo que el lienzo y las referencias no son dos capacidades separables y la primera se apoya en la segunda. Sobre ese vínculo hay dos propiedades que conviene no confundir, porque una es del usuario y la otra del sistema. La espera al destino, que es la que define el orden de arranque, la declara el administrador: el sistema la propone al trazar la flecha y él decide, en lugar de deducirla el sistema de qué dato se referencia. Y una arista puede existir sin ninguna variable de por medio, cuando su única razón es esa espera, lo que además permite importar sin pérdida un archivo de composición que declare orden explícito.

De esa distinción salen dos promesas de alcance que antes no existían. Renombrar un servicio no rompe lo que lo referencia, porque la referencia queda vinculada al servicio y no a su nombre: renombrar pasa a ser una operación de presentación. Y esperar a otro servicio deja de imponerse a quien no lo necesita, porque el que tiene reintentos propios puede no esperar aunque lo consuma.

Una precisión final, del 2026-07-28, que extiende esas promesas y suma F-25. La primera promesa deja de valer sólo para los servicios: renombrar cualquier elemento del modelo dejó de romper lo que le apunta, porque las relaciones se establecen por identidad y el nombre es un atributo. La segunda es nueva: dos variables compartidas pueden llamarse igual sin ambigüedad, de modo que el administrador ya no tiene que inventar un nombre distinto para evitar una colisión que no existe.

F-25 es la contrapartida de esas dos libertades y su diseño invierte el orden habitual. En vez de preguntarle al administrador antes de instanciar una plantilla, obligándolo a decidir sin ver el resultado, el sistema crea separado —que es la opción segura— y después le informa si detecta que probablemente convenga compartir. La decisión se toma con la información a la vista y es reversible. Por eso F-25 advierte y nunca bloquea: una condición de higiene no es un error, y tratarla como tal convertiría una ayuda en un obstáculo.
...
|---|---|---|---|
| Desarrollo | Entorno contenedorizado declarativo sobre el host de desarrollo, con acceso al motor de contenedores del propio host | Equipo de desarrollo y agente IA de codificación | El resultado se observa desde el navegador del host. El detalle de plataforma está en `Compatibilidad-Plataformas` |
...
|---|---|---|---|---|
| S-01 | Las métricas de éxito de negocio y sus umbrales | Confirmado el 2026-07-27 | Valores propuestos por el intake, adoptados sin cambios: son el objetivo de negocio de la solución | `Vision-Producto` §6 y los objetivos OBJ-01 a OBJ-04 |
| S-02 | Los umbrales mínimos de cobertura de pruebas por proyecto de código | Confirmado el 2026-07-27 | Los cuatro pares de umbrales se adoptan sin cambios y son control bloqueante del pipeline | `Acuerdo-Equipo` §5 y la categoría 08-Calidad-Y-Pruebas |
...
| S-04 | El esquema de versionado y la convención de mensajes de cambio, sobre el etiquetado por etapa que las fuentes sí declaran | Confirmado el 2026-07-27 | Esquema propuesto adoptado sin cambios | `Acuerdo-Equipo` §4.3 |
| S-05 | La ubicación del repositorio remoto | Resuelto con evidencia el 2026-07-27 | Deja de ser supuesto: el remoto existe y está configurado, y el dato es verificable en el entorno con `git remote get-url origin` en la raíz del repositorio destino | `Acuerdo-Equipo` §7 y la categoría 09-Devops |
| S-06 | Los nombres propios del propietario del problema y del agente humano del proyecto | Cerrado por identificación de rol el 2026-07-27 | No se aportan nombres propios y no se requieren: los actores se identifican por su rol, unívoco porque el propietario, el lead técnico y el usuario final son la misma persona. Ningún artefacto downstream debe pedir un nombre propio | `Vision-Producto` §2 y el README de esta sección |
...
Las dos pendientes de instanciación que esta sección registró después se cerraron también el 2026-07-28, y no del mismo modo, lo que conviene distinguir. La de la clave de variable compartida **desapareció**: al establecerse las relaciones por identidad, la clave de una compartida dejó de exigir unicidad, la plantilla crea su propio elemento y no hay colisión posible. No quedó nada que decidir. La del nombre de servicio **se resolvió con una regla**: sufijo automático más aviso al administrador. La unicidad del nombre se conserva porque cumple una función real —es el alias por el que el motor resuelve al servicio—, y sufijar ya no rompe nada, porque lo que referencia al servicio lo hace por identidad y no por nombre.

Queda un solo pendiente que alcance al alcance de esta solución: la matriz de navegadores. Este documento no le propone respuesta y declara su consumidor.

| ID | Supuesto abierto | Qué queda sin definir | Quién resuelve | Qué bloquea |
|---|---|---|---|---|
| Matriz de navegadores | Qué familias y versiones mínimas de navegador de escritorio se soportan | El intake declara la exigencia de conexión persistente por WebSockets, pero no la familia ni la versión mínima | Cliente | El alcance de lo que 03-UX-UI-DX puede prometer sobre el comportamiento de la interfaz, y la verificación de compatibilidad en 08-Calidad-Y-Pruebas y 09-Devops. Queda declarada como brecha en `Compatibilidad-Plataformas` §2.2 |
...
Qué no se registra acá, y dónde vive. El SOLUTION-INTAKE §19 lleva tres registros distintos de cosas sin cerrar y enumera cinco pendientes de decisión, de las cuales esta sección recoge una. De las otras cuatro, tres son de interfaz y las resuelve 03-UX-UI-DX, con 02-Especificacion-Funcional en dos de ellas —cómo se distinguen en el lienzo las aristas que declaran espera, el maquetado del paso de clasificación de variables y de la pantalla de variables compartidas, y si la interfaz señala un literal que duplica un valor que el sistema ya provee—; y la cuarta es la ubicación de F-23, F-24 y F-25 en una fase, que resuelven `Roadmap-Producto` §2.4 y la categoría 07-Plan-Sprint. Ninguna de las cuatro acota qué entra o qué sale del alcance, que es el objeto de este documento.

El intake lleva además un tercer registro, DI-01 a DI-16, con las especificaciones que el orquestador derivó para hacer implementables las decisiones del 2026-07-28. Están aplicadas y la cadena puede consumirlas, pero son propuesta del integrador y no dato del cliente: las categorías 02 y 05 deben tratarlas como revisables. No se transcriben acá porque son de modelado y de contrato, no de alcance.

## 7. Restricciones

Las restricciones del cliente están enumeradas en `Vision-Producto` §7 como RE-01 a RE-12. Este documento agrega la lectura de cada una en clave de alcance.
...
| RE-10 | Una rama y un pull request por etapa, con etiqueta por etapa cerrada y ningún secreto en el repositorio | Sin efecto sobre las capacidades del producto, pero sí sobre los entregables: condiciona EN-03 y EN-07, y su detalle operativo vive en `Acuerdo-Equipo` §4 |
...
| CA-04 | Las reglas de negocio que introduce cada etapa están cubiertas por pruebas automatizadas | Verificación de cobertura en el control de calidad del pipeline, con los umbrales por proyecto de código declarados en `Acuerdo-Equipo` §5 |
...
| 5. Propagación | Un cambio aceptado obliga a revisar `Roadmap-Producto` y las categorías 01, 02 y 06. El cambio no se considera cerrado hasta que esa propagación está hecha |
...
| 3. Objetivos del proyecto | §8, a través de los objetivos SMART de `Vision-Producto` |
...
|---|---|
| 01-Necesidades-Negocio | Las capacidades F-01 a F-17 más F-23, F-24 y F-25 como origen de cada necesidad de negocio, y los objetivos OP-01 a OP-07 |
...
| 1.3 | 2026-07-28 | Propagación de la tercera pasada sobre el mecanismo de variables del SOLUTION-INTAKE. F-06 deja de decir que el orden de arranque se respeta por deducción del grafo y pasa a decir que el grafo lo declara. La precisión sobre F-04, F-06 y F-24 se reescribe: la espera al destino es una propiedad que el administrador declara y que el sistema propone, no algo que el sistema deduzca del dato referenciado, y una arista puede existir sin variable cuando su única razón es esa espera. Se incorporan las dos promesas de alcance que la pasada habilita y que antes no existían: renombrar un servicio no rompe lo que lo referencia, y esperar a otro servicio deja de imponerse a quien no lo necesita | Product Manager Senior (AG-00) con Analista de Negocio Senior (AG-01) |
| 1.4 | 2026-07-28 | Cuarta pasada del SOLUTION-INTAKE, de terminología. §2 desambigua el párrafo donde los dos sentidos convivían —el proyecto como emprendimiento y el proyecto SelfHosted como unidad de trabajo— y declara cuál designa «proyecto» a secas en el resto del documento. Se califican como proyecto de código las dos menciones que lo requerían: el umbral de cobertura de S-02 y el de CA-04. La cabecera califica el proyecto de código principal. Se adopta además la convención de citas de la solución: las citas en prosa van sin sufijo de versión y el nombre versionado queda reservado a la identidad del propio documento, a la trazabilidad upstream de la cabecera y a los enlaces navegables, que necesitan el nombre de archivo real. Es lo que corta la cascada por la que un documento subía de versión sin tener una línea propia que corregir. | Product Manager Senior (AG-00) con Analista de Negocio Senior (AG-01) |
| 1.5 | 2026-07-28 | Quinta y última pasada del SOLUTION-INTAKE. §4.1 suma F-25, higiene del modelo, con prioridad Could Have: las capacidades incluidas pasan de diecinueve a veinte. La nota de capacidades incorpora las dos promesas que el principio de identidad extiende —renombrar cualquier elemento dejó de romper lo que le apunta, y dos variables compartidas pueden llamarse igual— y el criterio de diseño de F-25, que invierte el orden: el sistema crea separado, que es lo seguro, y después informa, en lugar de obligar a decidir a ciegas antes de instanciar. §6.2 cierra las dos pendientes de instanciación y distingue cómo se cerró cada una, porque una desapareció por modelo y la otra se resolvió con una regla; queda un solo pendiente de alcance, la matriz de navegadores. Se actualizan los recuentos de los otros dos registros del intake | Product Manager Senior (AG-00) con Analista de Negocio Senior (AG-01) |
