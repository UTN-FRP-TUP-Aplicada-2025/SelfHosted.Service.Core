...
**Trazabilidad upstream:** PRODUCT-INTAKE-SelfHosted-Service (Parte A completa, §15, §17.P.9, §17.P.10, Parte E §22, §24), PRODUCT-MANIFEST-SelfHosted-Service §1 y §2
...
| --- | --- | --- | --- | --- |
| 1 | [Vision-Producto.md](Vision-Producto.md) | Por qué existe el producto, quiénes son sus stakeholders, qué promete, qué objetivos numéricos persigue, contra qué restricciones y con qué riesgos. Incluye el glosario del dominio | 2.0 | Propuesto |
| 2 | [Alcance-Producto.md](Alcance-Producto.md) | Qué capacidades entran, cuáles quedan explícitamente afuera y por qué, con qué supuestos, entregables, ambientes y criterios de aceptación, y cómo se gestiona un cambio de alcance | 2.0 | Propuesto |
| 3 | [Roadmap-Producto.md](Roadmap-Producto.md) | En qué orden se entrega: cinco fases, sus épicas, sus etapas, sus dependencias y los criterios verificables de transición entre fases | 2.0 | Propuesto |
| 4 | [Compatibilidad-Plataformas.md](Compatibilidad-Plataformas.md) | Sobre qué plataformas corre, se construye y se opera el producto, con versiones mínimas y motivos, y qué riesgos de plataforma quedan abiertos | 2.0 | Propuesto |
| 5 | [Acuerdo-Equipo.md](Acuerdo-Equipo.md) | Cómo trabaja el equipo: roles y autoridad, ceremonias, veintiocho acuerdos verificables, definición de terminado y definición de listo | 2.0 | Propuesto |

El segundo documento se llamaba `Alcance-Proyecto.md` hasta la migración normativa al conjunto 6.0. El renombre a `Alcance-Producto.md` está declarado en el bloque de impacto sobre destinos existentes de la entrada `[5.0]` del framework y no se resolvió por inferencia: el nombre viejo nombraba con «proyecto» algo que es de nivel producto. La copia con el nombre anterior queda archivada en `_legacy/` y no se toca.
...
Un lector que sólo necesite entender por qué existe el producto puede leer Visión de Producto §1, §3 y §9. Un lector que vaya a planificar necesita además Alcance del Producto §4 y §5, y el Roadmap completo.
...
| Usuario final: administrador del producto, único usuario con credenciales | Beneficiario | Visión de Producto §2.1 |
...
El Product Owner no figura en esta tabla porque no es una categoría de la tríada de stakeholders sino un rol humano que el framework ubica fuera de la cadena de subagentes, dueño de la priorización y de las exclusiones. La cabecera del intake lo declara desde su versión 2.3 por derivación, y el agente humano del proyecto lo confirmó el 2026-07-30 al resolver la entrada `[R-1]` de la batería de la fase M2 de la migración normativa: en este producto coincide con la misma persona que cumple los tres roles de arriba. Es el destinatario de las brechas de §6 que requieren una decisión de producto.
...
| --- | --- | --- |
| Confirmación de OBJ-05, el objetivo de escala operable sin degradación | Visión de Producto §5; Alcance del Producto §6.3 | Agente humano del proyecto, en el próximo punto de control |
| Asignación de las épicas EP-23, EP-24 y EP-25 —variables compartidas, referencias entre variables e higiene del modelo— a una fase y a un corte vertical | Roadmap de Producto §2.5 y §2.6; Alcance del Producto §6.3 | 07-Plan-Sprint, con decisión del agente humano del proyecto |
...
| Detección de un literal que duplica un valor provisto por el sistema, y con qué forma | Alcance del Producto §6.3 | 03-UX-UI-DX y 02-Especificacion-Funcional |
| Distinción visual entre las aristas que declaran espera y las que no, incluida la arista sin variable | Alcance del Producto §6.3 | 03-UX-UI-DX y la maqueta de validación visual |
| Maquetado del paso de clasificación de variables de la adopción y de la pantalla de variables compartidas del proyecto SelfHosted | Alcance del Producto §6.3 | 03-UX-UI-DX |
| Modelado de los tres objetos declarados y no diseñados: el secreto, la red del proyecto SelfHosted y el volumen o directorio al que apunta un montaje | Alcance del Producto §6.3 | 05-Arquitectura-Tecnica, que recibe la prueba con la que decidir y no la decisión |
| Revisión de las catorce especificaciones de integración `DI-02` y `DI-04` a `DI-16`, aplicadas y sin revisar | Alcance del Producto §6.2 | Agente humano del proyecto; consumidas como revisables por 02-Especificacion-Funcional, 05-Arquitectura-Tecnica y 08-Calidad-Y-Pruebas |
...
| 2.0 | 2026-07-30 | Migración normativa del conjunto 4.1 al 6.0, fase M4, bajo `Rules-Contexto` 3.1 y `Vocabulario-Rules` 2.1. Clasificación **regenerar contenido** por el salto major de la regla que lo gobierna; fuente de contenido: el documento de origen, archivado en `_legacy/2026-07-30/`, más los cinco documentos hermanos para la columna de versión de §2. Sube **major** porque la nomenclatura anterior deja de cumplir. La cabecera pasa de la etiqueta `Proyecto` a `Producto` y el identificador `Nombre-Solucion` a `Slug-Producto`; la trazabilidad upstream apunta al `PRODUCT-INTAKE-SelfHosted-Service` y al `PRODUCT-MANIFEST-SelfHosted-Service` renombrados. **§2 registra el renombre de `Alcance-Proyecto.md` a `Alcance-Producto.md`**, leído del bloque de impacto sobre destinos existentes de la entrada `[5.0]` y no inferido, y actualiza el enlace, el nombre y la columna de versión de los cinco documentos, que pasan a 2.0 por la misma migración. §5 convención 1 remite al glosario del dominio de `Vision-Producto` §9 como glosario raíz de la cadena, según el criterio de gobierno del glosario que `Rules-Contexto` 3.1 §6 incorpora, y declara que en contexto de proceso «proyecto» a secas es la forma correcta; §5 convención 2 declara el nivel de aplicación producto de la categoría y la regla R3 de `Vocabulario-Rules`. §4 registra que el Product Owner quedó **confirmado el 2026-07-30**, según la cabecera del intake migrado, y deja de estar pendiente. Sustitución léxica **por ocurrencia** según `Vocabulario-Rules` §9.5 y el plan de migración §3.5: de las siete ocurrencias de la cadena `soluci`, una era el identificador de cabecera y seis designaban el nivel superior y pasan a «producto» con su concordancia de género —§3.1 dos veces, §4 dos veces y §5 convención 2 y convención 4—. No hay ninguna «resolución» en este documento. De las ocurrencias de «proyecto», **ninguna pasó a «proyecto de código»** que no lo fuera ya; las de «proyecto SelfHosted» son la entidad del dominio, las de «agente humano del proyecto» son el emprendimiento, y las de la convención 1 de §5 son menciones del término y no usos. Ninguna brecha de §6, ningún stakeholder y ninguna decisión de inclusión cambió | Product Manager Senior (AG-00) |
