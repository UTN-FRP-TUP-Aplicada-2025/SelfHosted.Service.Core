...
**Trazabilidad upstream:** PRODUCT-INTAKE-SelfHosted-Service §1, §4, §6, §9, §10, §12, §15, §15.1, §19, §22.2, §22.3, §24.1, §24.3, y la sección «Supuestos registrados por este intake y su estado»
...
La entrega se organiza en cuatro alcances incrementales declarados por el cliente. El Alcance 1 es el mínimo sin el cual el producto no resuelve el problema; los tres siguientes agregan observabilidad, portabilidad y automatización. Cada alcance se corta en etapas, y cada etapa entrega una funcionalidad acotada operativa de punta a punta.
...
Las definiciones canónicas viven en el glosario del dominio de [Visión de Producto](Vision-Producto.md) §9, que `Rules-Contexto` 3.1 §6 declara glosario raíz de la cadena: ningún documento de esta categoría redefine por su cuenta un término que la categoría acuña o precisa. Lo que esta tabla registra es cómo se leen esos términos dentro de este documento.
...
| --- | --- | --- |
| EN-01 | Aplicación desplegable en el servidor de referencia, único artefacto ejecutable del producto | Agente humano del proyecto |
| EN-02 | Especificación del producto: el conjunto de categorías de documentación generadas bajo `SDD/Docs/` | Equipo de desarrollo y agente IA de codificación |
...
| EN-07 | Registro de cambios del producto, actualizado en la rama de cada etapa | Agente humano del proyecto |
...
| Producción | El producto corriendo como un contenedor más en el servidor de referencia | Agente humano del proyecto |
...
| RE-06 | El producto debe ser liviano: presupuesto de memoria de cientos de MB y sin sondeo agresivo de métricas. Ninguna capacidad puede violarlo |
...
| §3 Objetivos del producto | §22.3 [FA] |
...
| §5 Alcance excluido | §9 Lo que NO es este producto; §4 (F-18 a F-22) |
...
| 2.0 | 2026-07-30 | Migración normativa de `Rules-Contexto` 2.1 a 3.1, fase M4 del plan [`Plan-Migracion-4.1-a-6.0.md`](../Audit/Plan-Migracion-4.1-a-6.0.md). Salto major, clasificación «regenerar contenido», fuente de contenido: el documento de origen `Alcance-Proyecto.md`, archivado en [`_legacy/2026-07-30/Alcance-Proyecto-v1.0.md`](_legacy/2026-07-30/Alcance-Proyecto-v1.0.md). Sube major porque el artefacto cambia de nombre, el vocabulario normativo y los identificadores de la cabecera cambian, y la versión anterior deja de cumplir. **Renombre de artefacto** de `Alcance-Proyecto.md` a `Alcance-Producto.md`, leído del bloque de impacto sobre destinos existentes de la entrada `[5.0]` del `CHANGELOG.md` del framework, cuyo propósito declarado es eliminar el sentido «emprendimiento» del árbol de artefactos; el archivo con el nombre legado se borra y no se deja duplicado. Estructura: por `Rules-Contexto` 3.1 §4.2, §3 pasa de «Objetivos del proyecto» a «Objetivos del producto» y §8 de «Criterios de aceptación del proyecto» a «Criterios de aceptación del producto», con la tabla de contenido y la tabla de trazabilidad upstream reconciliadas contra los títulos nuevos. Cabecera: la etiqueta `**Proyecto:**` pasa a `**Producto:**` sobre el mismo valor de plano producto, el identificador `Nombre-Solucion` pasa a `Slug-Producto` y la trazabilidad upstream pasa a nombrar al `PRODUCT-INTAKE-SelfHosted-Service`; por `Vocabulario-Rules.md` §4 R3 la cabecera de una categoría de nivel producto no declara proyecto de código, y esta no lo declaraba. Vocabulario: siete ocurrencias de «solución» que designaban el nivel superior pasan a «producto», con la concordancia de género rehecha en cada una —artículo, demostrativo y adjetivo, incluida «La solución debe ser liviana» que pasa a «El producto debe ser liviano»—; las dos «resolución» quedan intactas y ninguna sustitución se hizo por reemplazo global de cadena, según `Vocabulario-Rules.md` §9.5. El documento de origen tenía cuarenta y una ocurrencias de «proyecto» a secas: se clasificaron una por una y se sustituyeron diez, todas ellas de nombre de artefacto, de título de sección exigido por la normativa vigente, de etiqueta de cabecera o de ancla de la tabla de contenido. Las treinta y una restantes quedan intactas, porque designan el emprendimiento o la entidad del dominio, y el PRODUCT-INTAKE §12 declara que convertir el sentido de proceso produciría una afirmación falsa; ninguna se convirtió a «proyecto de código». §2.2: por el criterio de gobierno del glosario de `Rules-Contexto` 3.1 §6, la tabla deja de ser una definición propia y remite a [Visión de Producto](Vision-Producto.md) §9 como glosario raíz, conservando su contenido como clave de lectura de este documento. Ningún enunciado de alcance, capacidad, exclusión, supuesto, brecha, restricción o criterio de aceptación se modificó | Product Manager Senior (AG-00) |
