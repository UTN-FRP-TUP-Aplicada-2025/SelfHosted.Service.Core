...
**Brecha declarada:** siete decisiones del intake §19 —`Q-15` a `Q-21`— condicionan qué se hace con esta entidad, y ninguna está cerrada. La más determinante es `Q-15`, si el despliegue registra el digesto: **sin ella la entidad existe en el modelo y nadie la escribe**. Ver §9.
...
| Despliegue | Imagen usada | El **digesto** y la referencia legible de la imagen con la que se creó el contenedor | Es lo que permite saber qué corrió exactamente y volver a ese despliegue (CU-38). **Depende de `Q-15`**, abierta |
...
| Imagen | Marca de conservada | Declara que la imagen está protegida de la limpieza | Una imagen conservada no se limpia aunque ningún despliegue activo la referencie (RN-40) |
...
| Vía de alta | Cómo llega el usuario a crear un servicio. Son siete y **no se persisten**: son cómo se llegó, no qué quedó. Dos de ellas —incorporar un contenedor existente y tomar un ítem del catálogo— no tienen origen propio y dejan huella en la **procedencia** del servicio. **No confundir con origen** |
...
| Plantilla | Un ítem del catálogo, visto desde lo que declara: **el alta de un servicio sin la cola de despliegue**, con sus valores variables convertidos en parámetros. No se instancia, no se despliega y no ocupa dirección |
| Versión de contenido | Versión de lo que el usuario publicó de un ítem del catálogo. Se incrementa al editar un ítem ya publicado, y es lo que el servicio instanciado **copia** como procedencia. **No confundir con versión de formato** |
| Versión de formato | Versión de la forma del archivo del catálogo, no de su contenido. Cambia cuando cambia el esquema, y es lo que permite convertir un catálogo importado sin adivinar su forma. **No confundir con versión de contenido** |
| Imagen | La imagen de contenedor tratada como **objeto con identidad**: se referencia desde el despliegue, sobrevive al servicio que la construyó y tiene ciclo de vida propio. Lleva marca de **pertenencia**, que distingue lo que el producto administra de lo **ajeno** que ve y no administra, y marca de **conservada**, que la protege de la limpieza |
| Digesto | Identidad real de una imagen, calculada sobre su contenido. Es lo que permite saber **qué corre exactamente**, incluso cuando la etiqueta es flotante y hoy apunta a algo distinto de lo que apuntaba ayer. La **etiqueta** es un nombre reasignable y no identifica nada de forma estable |
| Borrador | Estado de configuración de un servicio que existe, es visible en el lienzo y está incompleto de forma visible. **No entra al conjunto de cambios pendientes y no es aplicable.** Es lo que hace utilizable guardar a mitad de camino |
...
| Imagen | CU-13, CU-15, CU-37, CU-38 | RN-40 |
...
| **Siete decisiones abiertas condicionan qué se hace con la entidad Imagen**: `Q-15` a `Q-21`. La más determinante es `Q-15`, si el despliegue registra el digesto: sin ella **la entidad existe en el modelo y nadie la escribe**, y CU-37 y CU-38 no son implementables. Las siete se cierran con esa una | Intake §19, tabla de pendientes de decisión; anexo E-23 | Agente humano del proyecto |
...
| **Ocho de las restricciones que sostienen este modelo son especificaciones de integración `[D-i]` de la versión 2.4 del intake y nacen sin revisar**: `DI-17` a `DI-24`. Alcanzan a la variante discriminada de origen, al estado de configuración del servicio, a la procedencia, a las dos verificaciones, y a RC-19. Se consumen declarándolas revisables | Intake §19, tabla de especificaciones de integración | Agente humano del proyecto |
