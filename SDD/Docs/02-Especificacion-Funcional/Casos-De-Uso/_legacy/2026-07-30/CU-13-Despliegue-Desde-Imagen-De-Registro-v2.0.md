> **Documento archivado.** Estado: **Superado**. Es la copia completa del estado previo de `CU-13-Despliegue-Desde-Imagen-De-Registro.md`, versión **2.0**, archivada el 2026-07-30 por la política de deprecación de `Master-Prompt.md` §5.1 al incorporarse la ronda de decisiones del agente humano del proyecto del 2026-07-30 —`Q-15`, `Q-17`, `Q-27` y la confirmación de `DI-17` a `DI-19`—. La versión vigente es [`CU-13-Despliegue-Desde-Imagen-De-Registro.md`](../../CU-13-Despliegue-Desde-Imagen-De-Registro.md). **El cuerpo que sigue no se modificó.**
>
...
5. El sistema **registra el digesto de la imagen obtenida en el despliegue**, que es su identidad real y lo que permite saber qué corre exactamente incluso cuando la etiqueta es flotante. **Depende de la decisión abierta `Q-15`**: ver §10.
...
- **Las dos variantes de imagen se alinearon en la versión 1.1.** El origen por imagen dejó de ser un valor único y pasó a ser dos variantes discriminadas, `imagen-publica` e `imagen-privada`, que difieren en dos campos: el registro pasa de selector a dirección, y aparece la credencial de registro. **No son dos naturalezas**: es el mismo origen con y sin autenticación, y este caso de uso las cubre las dos. Es la especificación de integración `DI-18`, **sin revisar**.
- **Brecha declarada, `Q-15`:** el registro del digesto por despliegue del paso 5 **depende de una decisión abierta**. Sin ella, el usuario ve una etiqueta y no sabe qué imagen corre; y sin ella tampoco es posible volver a un despliegue anterior (CU-38) ni administrar el ciclo de vida de las imágenes (CU-37). El intake §19 la declara como la pendiente de mayor palanca de las diecisiete abiertas. Destinatario: agente humano del proyecto.
- **Brecha declarada, `Q-6`:** si el despliegue registra además qué commit construyó la imagen. El digesto de `Q-15` la responde para el origen por imagen; para el origen por repositorio remoto la responde CU-15. Destinatario: agente humano del proyecto.
