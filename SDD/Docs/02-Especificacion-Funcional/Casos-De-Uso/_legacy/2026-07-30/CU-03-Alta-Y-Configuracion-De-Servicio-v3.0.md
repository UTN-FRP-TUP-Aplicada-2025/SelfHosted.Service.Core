> **Documento archivado.** Estado: **Superado**. Es la copia completa del estado previo de `CU-03-Alta-Y-Configuracion-De-Servicio.md`, versión **3.0**, archivada el 2026-07-30 por la política de deprecación de `Master-Prompt.md` §5.1 al incorporarse la ronda de decisiones del agente humano del proyecto del 2026-07-30 —`Q-15`, `Q-17`, `Q-27` y la confirmación de `DI-17` a `DI-19`—. La versión vigente es [`CU-03-Alta-Y-Configuracion-De-Servicio.md`](../../CU-03-Alta-Y-Configuracion-De-Servicio.md). **El cuerpo que sigue no se modificó.**
>
...
4. **El administrador resuelve el origen según la vía elegida.** Lo que se declara acá es el delta de cada vía, y está en la tabla de FA-01. El origen queda como una de las cinco variantes discriminadas que el anexo E-2 §20.2.3 declara, y cada variante exige sus datos y ninguno de otra variante (RN-08).
...
| **Imagen de registro público** | Imagen, etiqueta, política de actualización y registro como selector | Que la imagen y la etiqueta existan; devuelve el **digesto** | Imagen de registro público |
| **Imagen de registro privado** | Lo mismo, con el registro como dirección más la **credencial de registro** declarada en el alta | Lo mismo, **más que la credencial autentique** | Imagen de registro privado |
...
- **El reparto de siete vías sobre cinco variantes es propuesta del integrador, sin confirmar.** Son las especificaciones de integración `DI-17`, `DI-18` y `DI-19` del intake §19: la taxonomía de vías, la separación entre imagen de registro público y privado, y la existencia del servicio sin origen como nodo borrador. Se consumen **declarándolas revisables** y nunca como requisito cerrado del cliente. Si el agente humano del proyecto prefiere otro reparto, cambia este caso de uso sin reabrir nada más.
- **Brecha declarada, `Q-28`:** no hay camino para cambiar el origen de un servicio existente. La reentrada de FA-05 arranca en el paso 6 y excluye el origen; corregir una etiqueta mal escrita no tiene flujo. Destinatario: agente humano del proyecto.
- **Brecha declarada, `Q-27`:** el producto no declara si existe alguna forma de explorar un registro de imágenes. Sin catálogo de fábrica —decisión D-16— y sin exploración, un administrador que no sabe la dirección de la imagen **no tiene camino** en el primer minuto de uso. Las dos respuestas son legítimas; dejarlo sin declarar no. Destinatario: agente humano del proyecto, y luego 03-UX-UI-DX.
