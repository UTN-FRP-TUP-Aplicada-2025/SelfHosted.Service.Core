...
| Superficie en 03 | **`SUP-19`, exploración de registro de imágenes, emitida.** El identificador se propuso acá como el siguiente libre de la serie y lo emitió esa categoría, con wireframe propio, tal como la decisión de `Q-27` declara: [`Wireframes-Exploracion-De-Registro-De-Imagenes.md`](../../03-UX-UI-DX/Wireframes/Wireframes-Exploracion-De-Registro-De-Imagenes.md) versión 1.0, con el nombre canónico `Exploración de registro de imágenes` en su §1. `03-UX-UI-DX` pasa a **diecinueve** superficies. El wireframe traza de vuelta a este caso de uso y a CU-03, de modo que el vínculo resuelve en las dos direcciones |
...
- **Brecha declarada: dónde se configura el registro explorable.** Ninguna fuente declara si el conjunto de registros de imágenes es configuración de la instalación, del proyecto SelfHosted o del servicio, ni si el registro público de referencia viene configurado de fábrica. La precondición de §3 lo exige y no lo resuelve. **Sigue abierta**, y la emisión de `SUP-19` no la cierra: la superficie declara el mismo hueco. Destinatario: agente humano del proyecto.
...
- **Brecha cerrada el 2026-07-30: `SUP-19` está emitida.** La versión 1.0 de este caso de uso declaraba que la superficie no existía y que `03-UX-UI-DX` declaraba dieciocho, lo que era cierto al escribirlo: esta categoría se emitió antes que 03 dentro de la misma fase. **Esa categoría la emitió el mismo día** —[`Wireframes-Exploracion-De-Registro-De-Imagenes.md`](../../03-UX-UI-DX/Wireframes/Wireframes-Exploracion-De-Registro-De-Imagenes.md) 1.0, con `SUP-19` declarado en su §1— y pasa a diecinueve superficies. Es la brecha `B-25` del índice maestro, **cerrada**. **Lo que queda abierto de la consecuencia de `Q-27` no es la superficie sino su configuración**, y vive en las dos brechas de abajo y en `B-UX-29` de 03.
...
| --- | --- | --- |
| 1.1 | 2026-07-30 | **Corrección del hallazgo `P1-1`** del informe [`B2-Retroalimentacion-Decisiones-2026-07-30-r1.md`](../../Audit/B2-Retroalimentacion-Decisiones-2026-07-30-r1.md), por la política de versionado de `Master-Prompt.md` §5. Sube **minor**: ningún propósito, actor, precondición, paso de flujo, flujo alternativo, excepción, postcondición ni criterio de aceptación cambia. **Qué levantaba el hallazgo:** §9 y §10 declaraban que `SUP-19` no existía y que `03-UX-UI-DX` declaraba dieciocho superficies. **Verificado en disco antes de escribir, y no tomado del informe:** `SDD/Docs/03-UX-UI-DX/Wireframes/` tiene **19** archivos, entre ellos `Wireframes-Exploracion-De-Registro-De-Imagenes.md` versión **1.0**, cuyo §1 declara el nombre canónico `Exploración de registro de imágenes` y el identificador `SUP-19`. La fila de §9 pasa de «superficie prevista» a **superficie emitida**, con enlace relativo al wireframe y la constancia de que el vínculo resuelve en las dos direcciones; la brecha de §10 pasa a **cerrada**; y la nota de cabecera deja de decir «sin emitirla». **La afirmación era cierta al escribirse**: este caso de uso se emitió antes que la superficie dentro de la misma fase, y el informe declara el desfase como estructural y no accidental. **Las otras tres brechas de §10 siguen abiertas y ninguna se cerró por arrastre**: la de la configuración del registro explorable lo declara explícitamente, porque la superficie emitida declara el mismo hueco —es `B-UX-29` en 03 y `B-27` en el índice maestro de esta categoría—. La versión 1.0 queda archivada en `_legacy/2026-07-30/CU-39-Exploracion-Del-Registro-De-Imagenes-v1.0.md` |
