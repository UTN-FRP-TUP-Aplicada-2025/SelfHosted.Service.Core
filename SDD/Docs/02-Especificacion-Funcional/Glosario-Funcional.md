...
La verificación se corrió **por ocurrencia sobre los archivos de la categoría**, sin las copias de `_legacy/`, el 2026-07-30. La columna «Artefactos de 02 donde aparece» de §2 es el resultado de ese barrido y no una estimación: cada identificador que figura ahí se verificó en disco. Cuando el barrido devolvió un conjunto que **tiene nombre** —los CU, las RN, las RC—, la celda lo declara en lugar de enumerarlo y dice cuál es; en los demás casos enumera, por largo que sea el listado.

**El corpus pasó de cien a ciento un archivos el 2026-07-30**, con la emisión de `CU-39`. Los conteos de las celdas de §2 que no lo nombran **no se recalcularon uno por uno**: se verificaron las filas que `CU-39` alcanza y se dejaron intactas las demás, porque un conteo que no cambió no necesita reescribirse. Las filas alcanzadas llevan la marca **`[+Q-27]`** en la primera columna.
...
| Los **ciento un artefactos hermanos** de 02 —39 CU desde el 2026-07-30, 40 RN, 19 RC, el modelo conceptual, el índice maestro y el `README.md` de la sección— | Los términos que el cuerpo de la categoría usa. Cada definición de §2 sale de **cómo los artefactos usan el término**, no de una redacción nueva |
...
**Ciento nueve términos.** Una fila por término: término canónico, definición operativa, artefactos de 02 donde aparece, y sinónimos o alias cuando los hay. Las diez subsecciones agrupan por área del dominio y **no alteran la regla**: la tabla es una sola y cada término tiene exactamente una fila. El reparto por subsección es 7 + 32 + 23 + 13 + 8 + 6 + 5 + 6 + 4 + 5.

Los **veinticuatro** términos que la versión 1.0 sumó al cerrar el hallazgo `P1-r2-02` llevan la marca **`[+P1-r2-02]`** en la primera columna, para que la fila del control de cambios sea localizable en la tabla y no sólo enumerable en §6. Los **tres** que la versión 1.1 sumó por la decisión `Q-27` del 2026-07-30, y las **siete filas preexistentes que esa ronda de decisiones obligó a reescribir**, llevan la marca **`[+Q-27]`** con el mismo criterio.
...
| Término | Definición operativa | Artefactos de 02 donde aparece | Sinónimos o alias |
| --- | --- | --- | --- |
| **Vía de alta** `[+Q-27]` | Cómo llega el usuario a crear un servicio. Son **siete** y **no se persisten**: son cómo se llegó, no qué quedó. Dos de ellas —incorporar un contenedor existente y tomar un ítem del catálogo— no tienen origen propio y dejan huella en la **procedencia** del servicio. **No confundir con origen**, y tampoco con la **exploración de registro de imágenes**, que es un punto de entrada al paso del origen de dos de las siete y no una vía más. El reparto de siete vías sobre cinco variantes es **decisión cerrada** desde el 2026-07-30 | CU-03, CU-16, CU-17, CU-39, IDX, MC | — |
...
| **Verificación del origen** `[+Q-27]` | Consulta que el sistema hace al sistema externo que la vía de alta determina, para comprobar que el origen declarado existe y es alcanzable. Distingue «el dato es incorrecto» de «no se pudo verificar», que son situaciones distintas para el usuario | CU-03, CU-15, CU-39, MC, RN-08 | — |
| **Registro de imágenes** `[+Q-27]` | El servidor externo del que se descarga una imagen de contenedor, en sus dos formas: **público**, que no exige credencial, y **privado**, que sí. Es uno de los cinco referentes de «registro»: ver [§3.1](#31-registro--cinco-referentes). Es el sistema externo que la verificación del origen consulta y el que la **exploración de registro de imágenes** recorre; el producto lo lee y **no es su dueño** | CU-03, CU-08, CU-13, CU-17, CU-37, CU-38, CU-39, IDX, MC, RME, RN-08 | «el registro», nunca a secas: la forma desnuda se admite sólo para el registro del sistema |
| **Exploración de registro de imágenes** `[+Q-27]` | Operación de **sólo lectura** con la que el administrador recorre o busca lo que un registro de imágenes configurado publica, y vuelve con la referencia completa —registro, imagen y etiqueta— en lugar de escribirla de memoria. Es **punto de entrada al paso del origen** de las dos vías de imagen y **no una octava vía de alta**: no cambia el origen resultante ni deja huella en la procedencia. **No reemplaza a la verificación del origen**, que corre igual | CU-03, CU-13, CU-17, CU-39, IDX, MC, RME | «explorar el registro» |
| **Credencial de registro** `[+Q-27]` | Credencial con la que el producto se autentica contra un **registro de imágenes privado**. Es la que distingue la variante de origen privada de la pública, junto con el registro como dirección, y es la identidad con la que se descarga la imagen, se verifica el origen y se explora el registro. **Es una entidad distinta de la credencial de repositorio**, que es la del origen por repositorio remoto, y nunca se devuelve en claro | CU-03, CU-13, CU-39, IDX, RN-08, RN-15 | — |
...
| **Digesto** `[+Q-27]` | Identidad real de una imagen, calculada sobre su contenido. Es lo que permite saber **qué corre exactamente**, incluso cuando la etiqueta es flotante y hoy apunta a algo distinto de lo que apuntaba ayer. **El despliegue lo registra**: es dato decidido desde el 2026-07-30 y lo escriben CU-13 y CU-15, con lo que el bloque de imagen del despliegue tiene quien lo puebla | CU-03, CU-13, CU-15, CU-37, CU-38, CU-39, IDX, MC, RN-40 | — |
...
| Término | Definición operativa | Artefactos de 02 donde aparece | Sinónimos o alias |
| --- | --- | --- | --- |
| **Almacén de imágenes** `[+Q-27]` | El conjunto de imágenes que el motor de contenedores tiene en el host. Es la fuente del inventario, y el producto lo lee sin ser su dueño. **No confundir con el registro de imágenes**, que es remoto y es de donde las imágenes se descargan | CU-37, CU-38, MC, RN-40 | — |
...
| **Conservada** `[+Q-27]` | Marca que protege a una imagen de la limpieza, y que se puede retirar. Una imagen conservada **no se limpia aunque ningún despliegue activo la referencie**, que es exactamente el caso que la marca existe para cubrir. **Quién puede ponerla y con qué alcance sigue sin decidirse** (`Q-21`), y la marca se declara igual porque la protección no depende de esa respuesta | CU-37, CU-38, IDX, MC, RN-40 | «marca de conservada», «imagen conservada» |
| **Limpieza** `[+Q-27]` | Borrado de las imágenes descartables del almacén, **excluyendo** las conservadas y las ajenas. Produce un informe de lo borrado y de lo protegido. Su disparo es **sugerido** desde el 2026-07-30: el sistema detecta espacio recuperable y **lo propone**, y el usuario confirma. Ni corre sola ni espera únicamente a que se la pida. **Qué hace descartable a una imagen no está declarado**, y el umbral de la sugerencia lo fija `03-UX-UI-DX` | CU-15, CU-37, CU-38, IDX, MC, RN-40 | «limpieza sugerida» |
| **Volver a un despliegue anterior** `[+Q-27]` | Operación que toma un despliegue anterior de la línea de tiempo de un servicio, resuelve **por digesto** la imagen que usó y crea un despliegue nuevo con ella. Es distinta del redespliegue, que vuelve a aplicar la configuración actual. **Es técnicamente posible desde el 2026-07-30**, porque el despliegue registra el digesto; **que el producto la ofrezca sigue sin decidirse** (`Q-19`), y las dos cosas no son la misma | CU-13, CU-38, IDX, RME, RN-40 | «vuelta a un despliegue anterior» |
...
**Es la familia más cargada de la categoría y la que más consecuencias tiene aguas abajo.** La entrada heredada del punto 6 del modelo conceptual declaraba **cuatro** referentes; la versión 1.0 **sumó el quinto**, porque la migración normativa al conjunto 6.0 renombró el actor de sistema que lo lleva. **La versión 1.1 no suma ninguno**: la exploración de registro de imágenes de `CU-39` opera sobre `R4`, que ya estaba declarado, y lo único que cambia es que ese referente pasa a tener fila propia en §2.2 y un artefacto más donde vive.
...
| R4 | El **servidor externo de imágenes** del que se descarga una imagen, en sus dos variantes pública y privada. **Desde el 2026-07-30 es además el objeto de una operación propia**, la exploración de `CU-39`, y por eso tiene fila propia en [§2.2](#22-servicio-origen-e-imagen) | `registro de imágenes`, y `imagen de registro` cuando lo que se nombra es la variante de origen | CU-13, CU-37, CU-38, CU-03, CU-08, CU-17, CU-39, MC, IDX, RME, RN-08 |
...
| **Explorador de registro de imágenes** `[+Q-27]` | `CU-39` | **1** | [CU-39](Casos-De-Uso/CU-39-Exploracion-Del-Registro-De-Imagenes.md) §2. Queda fuera por **dos motivos y no por uno**: vive en un solo artefacto, y es un **nombre de actor no humano acuñado por esta categoría** y no un término del dominio, con el mismo criterio con el que ninguno de los otros dieciséis figura acá. La convención completa está en `Especificacion-Funcional.md` §8 |
...
Se deja constancia de un límite del artefacto, **acotado el 2026-07-30**, que no es un pendiente de fuente sino una dependencia declarada aguas arriba. Siete entradas de §2.8 y de §2.2 —imagen, digesto, marca de pertenencia, conservada, limpieza, almacén de imágenes y volver a un despliegue anterior— describen vocabulario cuya exigibilidad dependía de siete decisiones abiertas. **Dos de esas decisiones cerraron**: `Q-15`, con lo que **digesto** deja de ser vocabulario sin escritor, y `Q-17`, con lo que **limpieza** deja de tener un disparo indeterminado. El estado por término queda así:

| Término | Estado de su exigibilidad al 2026-07-30 |
| --- | --- |
| Imagen, almacén de imágenes, imagen ajena, marca de pertenencia | La entidad y su lectura están declaradas; **la marca de pertenencia de lo construido sigue abierta** (`Q-16`) |
| Digesto | **Exigible.** `Q-15` cerró en positivo y el despliegue lo registra |
| Limpieza | **Disparo exigible** —sugerido, `Q-17`—; **el criterio de descarte no está declarado en ninguna fuente** y es la brecha `B-26` del índice maestro |
| Conservada | Declarada como protección; **quién la pone y con qué alcance sigue abierto** (`Q-21`) |
| Volver a un despliegue anterior | **Técnicamente posible y no ofrecida**: `Q-19` sigue abierta |
| Exploración de registro de imágenes, registro de imágenes, credencial de registro | **Exigibles.** `Q-27` cerró en positivo; lo que sigue abierto es dónde se configura el registro explorable y el ámbito de credencial de la operación (`B-27`) |

Las brechas que lo declaran son **B-22** —cerrada—, **B-23**, **B-24**, **B-26**, **B-27** y **B-28** del índice maestro. **Ninguna de las diez definiciones se inventó**: todas salen del punto 6 heredado, del cuerpo de `CU-37`, `CU-38` y `CU-39`, o del anexo E-23 del intake vía esos casos de uso.
...
| --- | --- | --- |
| 1.1 | 2026-07-30 | **Incorporación del vocabulario que la ronda de decisiones del agente humano del proyecto del 2026-07-30 acuña o precisa**, consolidada en el `PRODUCT-INTAKE-SelfHosted-Service` v3.2 §19 y anexo E-23, y del artefacto hermano nuevo `CU-39`. Sube **minor**: ninguna entrada se retira, ninguna cambia de referente y ninguna definición contradice a `Vision-Producto.md` §9. **Regla de inclusión de §3.3 aplicada al artefacto nuevo**: el corpus pasa de cien a **ciento un archivos** y §1.1 lo declara, junto con el criterio de no recalcular los conteos de las filas que `CU-39` no alcanza. **Tres términos nuevos**, los tres verificados en más de un artefacto: **Registro de imágenes** —el referente `R4` de la familia «registro», que hasta ahora se declaraba sólo en §3.1 y que pasa a tener fila propia porque desde `CU-39` es objeto de una operación y no sólo el origen de una descarga—, **Exploración de registro de imágenes** y **Credencial de registro**. Los tres salen del cuerpo de `CU-03`, `CU-13` y `CU-39` y de `RN-08`; **ninguno se redactó de cero**. **Siete filas preexistentes reescritas**, todas por cambio de estatus de la decisión que las sostenía y no por cambio de sentido: **Digesto**, que pasa a declarar que el despliegue lo registra (`Q-15`); **Limpieza**, que pasa a declarar el disparo **sugerido** (`Q-17`) y que el criterio de descarte no está declarado; **Conservada**, que declara que `Q-21` sigue abierta; **Volver a un despliegue anterior**, que deja de decir «no es implementable hoy» y pasa a distinguir **técnicamente posible** de **ofrecida**, que es la separación que `Q-19` mantiene abierta; **Vía de alta**, que precisa que explorar **no es una octava vía**; **Verificación del origen** y **Almacén de imágenes**, que suman artefacto y precisión. Las diez llevan la marca **`[+Q-27]`** en la primera columna. **§2 pasa de 106 a 109 términos** y su reparto de 7 + 29 + 23 + 13 + 8 + 6 + 5 + 6 + 4 + 5 a **7 + 32 + 23 + 13 + 8 + 6 + 5 + 6 + 4 + 5**; §5.2 pasa de «ciento seis» a «ciento nueve». **Polisemias: ninguna nueva.** Siguen siendo **ocho familias**: la exploración opera sobre el referente `R4` de «registro», que ya estaba declarado, y §3.1 lo declara explícitamente para que no se lea como un sexto referente; su fila de `R4` suma `CU-39` y el puntero a la fila nueva de §2.2. **§5.1 suma una constancia de descarte**: `Explorador de registro de imágenes` vive en un solo artefacto y **es un nombre de actor acuñado, no un término del dominio**, con el mismo criterio con el que ninguno de los otros dieciséis figura en la tabla. **§5.2 reescribe su constancia de límite**: la que la versión 1.0 declaraba sobre siete entradas de dos casos de uso «no implementables hoy» pasa a una tabla de **estado por término**, porque dos de las siete decisiones cerraron y las otras cinco no, y agrupar los diez términos bajo una sola dependencia sería falso. **Ninguna decisión abierta se cerró acá y ninguna definición se emitió como pendiente.** La versión 1.0 queda archivada en `_legacy/2026-07-30/Glosario-Funcional-v1.0.md` |
