> **Documento archivado.** Estado: **Superado**. Es la copia completa del estado previo de `README.md`, versión **2.0**, archivada el 2026-07-30 por la política de deprecación de `Master-Prompt.md` §5.1 al incorporarse la ronda de decisiones del agente humano del proyecto del 2026-07-30 —`Q-15`, `Q-17`, `Q-27` y la confirmación de `DI-17` a `DI-19`—. La versión vigente es [`README.md`](../../README.md). **El cuerpo que sigue no se modificó.**
>
...
Un revisor que llegue a una sección suelta y no reconozca un término conviene que empiece por [Glosario-Funcional.md](Glosario-Funcional.md), en particular por su §3: siete términos de esta categoría tienen **más de un referente**, y «registro» tiene cinco.
...
| [Glosario-Funcional.md](Glosario-Funcional.md) | Glosario funcional de la categoría: 82 términos, 7 familias polisémicas y 18 términos referenciados del glosario raíz |
| [Casos-De-Uso/](Casos-De-Uso/) | 38 casos de uso, uno por archivo |
...
Ningún archivo lleva sufijo de versión en el nombre: la versión vive en el campo de su cabecera. **Hay dos tandas de versiones superadas archivadas**, las dos en el `_legacy/` de la carpeta de cada artefacto: `_legacy/2026-07-29/`, por la corrección de las definiciones de servicio de la Fase B2, y `_legacy/2026-07-30/`, por la migración normativa del conjunto 4.1 al 6.0. El sufijo de versión aparece **sólo** en la copia archivada.

## 3. Casos de uso vigentes

Los **38** en estado `Propuesto`, versión **2.0**, con una excepción: [CU-03](Casos-De-Uso/CU-03-Alta-Y-Configuracion-De-Servicio.md) está en **3.0**, porque ya había subido a 2.0 en la Fase B2 por la redefinición del alta y la configuración de servicios. Agrupados por la necesidad de negocio que los origina.
...
Advertencia de consumo: **CU-37 y CU-38 no son implementables hoy** y los dos lo declaran en su cabecera. Dependen de las decisiones abiertas `Q-15` a `Q-21` del intake §19, y son además los dos casos de uso que **ninguna necesidad de negocio previó** en su §7 —brecha B-20 del índice maestro—. Su necesidad asignada es la que figura en los agrupamientos de arriba.
...
Advertencia de consumo: **veintidós de las veinticuatro especificaciones derivadas siguen sin revisar**. Toda parte de regla marcada `[D-i]` se consume declarándola revisable y nunca como requisito cerrado del cliente.
...
| --- | --- | --- |
| §2 | Tabla de términos que esta categoría acuña o precisa, con los artefactos de 02 donde aparece cada uno | **82** |
| §3 | Términos con más de un referente, con su forma calificada y la evidencia de colisión | **7** familias: registro (5 referentes), procedencia (3), ámbito (3), resolución (2), higiene (2), huérfano (2), etiqueta (2) |
...
| §5 | Constancias del barrido: candidatos descartados por vivir en un solo artefacto | **1** descartado |
...
| --- | --- |
| 03-UX-UI-DX | Los flujos principales y alternativos de cada CU, sin su detalle visual, que es de esa categoría; el [glosario funcional](Glosario-Funcional.md), que su propio `Glosario-UX.md` debe referenciar y no duplicar; y las tres brechas de maquetado que el anexo E-18 dejó abiertas (B-07) |
| 05-Arquitectura-Tecnica | El modelo conceptual y sus 19 reglas, la dimensión de componentes esperados de cada CU —que es referencia tentativa, igual que los **dieciséis** nombres de actor no humano acuñados por esta categoría según §8 del índice maestro, sobre veintidós distintos— y las **siete** brechas de diseño que esta categoría delega (B-06, B-08, B-10, B-14, B-15, B-16, B-17) |
| 06-Backlog-Tecnico | Las **139** historias de usuario previstas de la matriz de §6 del índice maestro, con su numeración provisional (B-19) |
...
| 08-Calidad-Y-Pruebas | Los criterios de aceptación de los **38** CU, las **40** RN con su caso del anexo E-22, las **seis** reglas sin caso ejecutable propio —RN-02, RN-08, RN-10, RN-38, RN-39 y RN-40— y la mitad de RN-28 que el anexo deja sin cubrir (B-05) |
