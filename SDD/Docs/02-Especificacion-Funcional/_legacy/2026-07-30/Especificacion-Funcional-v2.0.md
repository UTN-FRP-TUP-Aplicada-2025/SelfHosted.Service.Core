> **Documento archivado.** Estado: **Superado**. Es la copia completa del estado previo de `Especificacion-Funcional.md`, versión **2.0**, archivada el 2026-07-30 por la política de deprecación de `Master-Prompt.md` §5.1 al incorporarse la ronda de decisiones del agente humano del proyecto del 2026-07-30 —`Q-15`, `Q-17`, `Q-27` y la confirmación de `DI-17` a `DI-19`—. La versión vigente es [`Especificacion-Funcional.md`](../../Especificacion-Funcional.md). **El cuerpo que sigue no se modificó.**
>
...
**Treinta y ocho** casos de uso, **cuarenta** reglas de negocio, un modelo conceptual, **diecinueve** reglas conceptuales del modelo y el **glosario funcional** de la categoría, para el producto **SelfHosted Service**. Los conteos se verificaron contra el disco el 2026-07-30: 38 archivos en `Casos-De-Uso/`, 40 en `Reglas-De-Negocio/`, 19 en `Modelo-Datos/reglas-conceptuales-de-modelo/`, uno de modelo conceptual y uno de glosario, sin contar las copias de `_legacy/`.
...
**Treinta y seis** de los casos de uso son exactamente los que las ocho necesidades de negocio declaran previstos en su §7. No se inventó numeración nueva, no se reasignó ninguna y no falta ninguna; **sobran dos**, CU-37 y CU-38, y §7 declara ese desajuste con su motivo. Las cuarenta reglas de negocio son exactamente las del anexo E-16 del intake, transcriptas con su momento de validación y su respuesta ante incumplimiento.
...
**Seis nombres trazan y dieciséis están acuñados, sobre veintidós distintos.** El conteo se verificó extrayendo la tabla de §2 de los 38 casos de uso el 2026-07-30, y **corrige el que esta sección declaraba**: decía diecinueve nombres con trece acuñados, contando sobre 36 casos de uso, y quedó atrás del fix de la Fase B2. Las tres diferencias, todas con evidencia en disco:
...
Al mismo tiempo, el conteo de `Motor de contenedores` pasa de nueve a **once** casos de uso, por CU-37 y CU-38. Los trece nombres acuñados que esta sección ya enumeraba **siguen existiendo con el mismo conteo**, verificado uno por uno; ninguno desapareció.
...
**Consecuencia para 05-Arquitectura-Tecnica, que es la destinataria de la advertencia.** Los dieciséis nombres acuñados **no son componentes declarados y no condicionan la descomposición**. Lo que sí es dato declarado son los siete módulos de la capa de aplicación que el intake §17.P.2 enumera —proyectos, servicios y despliegues, descubrimiento y adopción, red y conflictos de IP, catálogo, observabilidad, e identidad y tokens—, que son otros y menos que los nombres de actor de esta categoría, y son ésos los que 05 debe tomar como punto de partida. La correspondencia entre un actor no humano de un caso de uso y el componente que lo realice la fija 05. La dimensión «Componentes esperados en 05» del §9 de los **38** casos de uso ya lleva la marca de referencia tentativa por el mismo motivo.
...
Ninguna ambigüedad bloqueó la emisión de un documento de esta categoría. Las situaciones en las que una fuente no declaraba el dato necesario se resolvieron declarándolas como brecha en §9, con su destinatario, y no completándolas con un valor plausible. **Son veinticuatro, una de ellas cerrada**, y el conteo se verificó contra las filas de §9: la cifra que esta sección declaraba, diecinueve, era la del recuento anterior al 2026-07-29 y quedó atrás de la ampliación que §9 ya registraba.
...
[Glosario-Funcional.md](Glosario-Funcional.md), versión 1.0, estado `Propuesto`. Es **artefacto propio y obligatorio para los ocho tipos D8** desde `Rules-Especificacion-Funcional` 4.0 §2.1, con las cinco secciones que fija su §4.2.4.
...
| --- | --- | --- |
| §2, tabla de términos | Términos que esta categoría acuña o precisa, con su definición operativa, los artefactos de 02 donde aparece cada uno y sus alias | **82** |
| §3, términos con más de un referente | Familias polisémicas con sus referentes, la forma calificada de cada uno y la evidencia de colisión por artefacto | **7** familias: registro (5 referentes), procedencia (3), ámbito (3), resolución (2), higiene (2), huérfano (2), etiqueta (2) |
| §3.8, criterio negativo | Polisemias verificadas que **no se califican**, por contextos disjuntos | **3** constancias |
...
| §5.1, descartados | Candidatos que viven en un solo artefacto y por lo tanto no entran, con su conteo | **1** |

**Regla de inclusión aplicada.** §3.3 de las reglas de la categoría: entra todo término del dominio que aparece en **más de un artefacto de 02**. La verificación se corrió por ocurrencia sobre los cien archivos de la categoría, sin `_legacy/`.
